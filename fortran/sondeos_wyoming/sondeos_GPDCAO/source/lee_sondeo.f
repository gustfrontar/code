...
c     Declaracion de variables
     ...
     & ,hrr(k),pr(k),tdr(k),qr(k)
      ...
      open(50,file=file_name,status='unknown')
        i=1
        !leo el encabezado que tiene siempre 9 lineas.
        do jj=1,9
        read(50,*)nada
        enddo
...
c     ESCRITURA DE DATOS EN FORMATO BINARIO
      do j=1,n
      if(j.eq.1.or.pr(j).ne.pr(j-1))then
      if(pr(j).ne.-999)then
       if(tcr(j).ne.-999)then  
        if(hrr(j).ne.-999)then
         write(60,70)pr(j),tcr(j),hrr(j)
        else
         if(pr(j).le.350)then
!Si la humedad es undef pero el nivel está por encima de 350 rellenamos la humedad con 0.
