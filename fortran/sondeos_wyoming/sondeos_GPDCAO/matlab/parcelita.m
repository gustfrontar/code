...
%El parametro ice, indica si se va a considerar la fase solida por debajo de los -20ºC o no
%ice2=1 se considera la fase solida, ice=0 no se considera. El default es 0.
...
Ci=2089.0;  %Calor especifico del hielo a -10ºC
...
for j=nsat:nsat+1
    ...
    %B.
         ...
         Tro(j)=T(j)*(1+Rv*r(j)/Rd)/(1+r(j)); %Temperatura de densidad.   
         alfa=1/(1+Rv*rs(j)/Rd);
         ...
         %reagrupar los terminos multiplicados por dT y los multiplicados por dP, lo que acompaña a dT se lo llamo B y lo que 
         %acompaña a dP se lo llamo A. 
