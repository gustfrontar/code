...
function out = emagrama(ThetaMin,ThetaMax,DeltaTheta,Deltar)
...
Pi=1000;    %Valor de presion de donde parten las curvas del emagrama
% Calculos iniciales
nP2=round(-(Pmin-1000)/deltaP2); %No de pasos de integracion para las curvas secas y de r constante
nP=round(-(Pmin-1000)/deltaP);   %N0 de pasos de integracion para las adiabaticas saturadas
...
for theta=ThetaMin:DeltaTheta:ThetaMax
    Tseca(1,theta)=theta; %La temperatura inicial esta dada por el valor de theta.
    P(1)=100000; %Las adiabaticas siempre empiezan en 1000 hPa.
for j=1:nP2
    Kapa2=Rd/Cpd;
...
%Los valores de las variables de humedad de saturacion se suponen iguales a los da la parcela por encima del NCA.
    ...
    semilogy(Tpseudo(:,theta),-(P/100),'--m')
