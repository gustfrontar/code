...
- **Git repositorios**: [pkgs-manager](https://gitlab.smn.gov.ar/dmsr/pkgs-manager.git)
...
```shell
cd /data/share/usr/prod/pkgs-manager/bin/run
./run.sh install obs_asim_arg4k /data/share/usr/prod/SAP.SMN-ANA
```
...
```shell
cd /data/share/usr/prod/pkgs-manager/bin/run
./run.sh update obs_asim_arg4k /data/share/usr/prod/SAP.SMN-ANA
 ```

### Rollback
Para realizar un rollback de una actualización a la versión anterior:

```shell
cd /data/share/usr/prod/pkgs-manager/bin/run
./run.sh rollback obs_asim_arg4k /data/share/usr/prod/SAP.SMN-ANA
 ```
