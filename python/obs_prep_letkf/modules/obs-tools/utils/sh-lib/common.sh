...
function job_load_config_exp {
   ...
   if [[ -f "$CONFIGDIR/experiment.dirs" ]]; then
      file=$CONFIGDIR/experiment.dirs
   else
   ...
   [[ ! -d $CONFIGDIR ]] && job_reply $PROC $S_ERROR "Directory not found: $CONFIGDIR"
