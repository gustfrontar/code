...
ctlg = common.merge_catalog(ctlg_process.sfcshp, 'obs', 'sfcshp')

def get_files(path, ini, end):

   # Load files in pd.DataFrame
   files = sorted(glob.glob('{}/asm_ship_*'.format(path)))
...
def read_data(filename):
   ...
   # Set variables
   column_input = ['Date', 'Time', 'ID', 'Lat', 'Lon', 't2', 'td2', 'psfc', 'wdir10', 'wspd10', 'Dummy']
   ...
   try:
      ...
      # RHum
      data['rh2'] = common_obs.calc_relhum_from_dewpoint(data.t2.values, data.td2.values)
