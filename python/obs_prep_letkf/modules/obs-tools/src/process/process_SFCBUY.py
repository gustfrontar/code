...
ctlg = common.merge_catalog(ctlg_process.sfcbuy, 'obs', 'sfcbuy')

def get_files(path, ini, end):

   # Load files in pd.DataFrame
   files = sorted(glob.glob('{}/asm_boyas_*'.format(path)))
...
def read_data(filename):
   ...
   # Set variables
   column_input = ['Date', 'Time', 'ID', 'Lat', 'Lon', 't2', 'td2', 'psfc', 'slp', 'wdir10', 'wspd10', 'rh2', 'Dummy']
   ...
   try:
      ...
      # Pressure
      data['psfc'] = data.slp.fillna(data.psfc, axis=0)

      # Wind
      data.wspd10 *= common_obs.KT_MS
      ...
      if data.rh2.isna().any():
         data = common_obs.calc_relhum_from_dewpoint_ifmissing(data)
...
def proc_filename(filename, ctlg, ini, end, slot, monit_file):
   ...
   # Filter data
   tmp = data.shape[0]
   data = common_obs.apply_filters(ctlg, data, ['Lon', 'Lat', 'Lev', 'DateTime'], [ini, end])
   if data.empty: return pd.DataFrame(), nin, code_error
