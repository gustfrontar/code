...
def tai93_to_utc(tai_time):
    """Convert TAI time given as datetime object to UTC time."""
    return tai_time - dTAI_UTC_from_tai(tai_time) + dTAI_UTC_from_utc(datetime(1993,1,1))


def utc_to_tai93(utc_time):
    """Convert UTC time given as datetime object to TAI time."""
    print(dTAI_UTC_from_utc(datetime(1993,1,1)))
    return utc_time + dTAI_UTC_from_utc(utc_time) - dTAI_UTC_from_utc(datetime(1993,1,1))
