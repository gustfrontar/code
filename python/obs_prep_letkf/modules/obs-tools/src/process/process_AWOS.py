...
ctlg = common.merge_catalog(ctlg_process.awos, 'obs', 'awos')
...
def main_asim(args):
   ...
   if ENVVARS['MONIT']:
      ...
      monit_file = f'{MONITDIR}/{ANA_DATE:{os.environ["DATEFOLDER_fmt"]}}/{EXPNAME_BASE}/'
      os.makedirs(monit_file, exist_ok=True)

      # Create files 
      monit_file = common_obs.monit_create_csv(ctlg, monit_file, ANA_DATE)
...
def main_calib(args):
   ...
   #column_write = ['Lon', 'Lat', 'Lev'] + ctlg['VARS']
   pathobs = f'{REPODIR}/{ctlg["name"]}'
   pathout = f'{OBSDIR}/{OBS_DATE:%Y%m%d_%H%M%S}/{EXPNAME_BASE}'
   os.makedirs(pathout, exist_ok=True)
   ...
   data = common_obs.filter_duplicates(data, ['Lon', 'Lat', 'ID'])
   ...
   data['Tipo'] = 'AWOS'
   data = data.round(2)
   fileout = f'{pathout}/{ctlg["name"]}_{OBS_DATE:%Y%m%d%H%M%S}.csv'
