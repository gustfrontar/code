...
def get_ADPAUT_data(OBSREPO, DATE_INI, DATE_END, VARS):
    ...
    arg_list = [(filename, DATE_INI, DATE_END, ADPAUT_fun.read_data, src.ADPAUT) for filename in file_list_ADPAUT['Path']]
...
def proc_filename(filename, ini, end, read_data, source):
...
def get_ADPSFC_data(OBSREPO, DATE_INI, DATE_END, VARS):
    ...
    arg_list = [(filename, DATE_INI, DATE_END, ADPSFC_fun.read_data, src.ADPSFC) for filename in file_list_ADPSFC['Path']]
    with mp.Pool(min(QCPROC, len(arg_list))) as pool:
        pool_out = pool.starmap(proc_filename, arg_list)
...
def get_AWOS_data(OBSREPO, DATE_INI, DATE_END, VARS):
    ...
    arg_list = [(filename, DATE_INI, DATE_END, AWOS_fun.read_data, src.AWOS) for filename in file_list_AWOS['Path']]
    with mp.Pool(min(QCPROC, len(arg_list))) as pool:
        pool_out = pool.starmap(proc_filename, arg_list)
...
if __name__ == "__main__":
    ...
    OBS_DATE = os.environ['START_TIME']
    OBS_HOUR = os.environ['START_HOUR']
    ...
    if not os.path.exists(filename_stationsQC):
        ...
        stations_QC_template = f'{os.environ["MAINDIR"]}/config_exp/stations.QC'
