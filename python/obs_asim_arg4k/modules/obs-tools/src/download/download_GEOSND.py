...
def main(args):
   ...
   # Output directory
   pathout = '{}/GEOSND'.format(os.environ['REPODIR'])
   ...
   data_folders = ['ABI-L2-LVMPF', 'ABI-L2-LVTPF']
   #ABI-L2-LVMPF - Legacy Atmospheric Moisture Profile
   #ABI-L2-LVTPF - Legacy Atmospheric Temperature Profile
   ...
   for win_date in dates:
      for folder in data_folders:
          ...
          if fs.exists(data_path): # newer versions of s3fs raise an exception if path doesn't exists
              # List contents of GOES-16 FULLDISK Temperature and Moisture profiles
...
if __name__ == '__main__':
   ...
   ut.set_walltime(src.GEOSND['WALLTIME'])
   try:
      main(sys.argv[1:])
