...
def proc_filename(filename, source, ini, end, slot, monit_file):
   ...
   if monit_file is not None:
      ut.do_monit(data, source, slot, 'REPO', monit_file, data['Prop_Name'].unique()[0])

   # Additional filters
   # 1) QC
   common_cols = [x for x in source['VARS'] if x in data.columns]
   obs_in = data.dropna(subset = common_cols, how = 'all').shape[0]
   for var, thrs in source['OTHER']['QC_THRESHOLD'].items():
