...
def proc_filename(file_data, source, monit_file):
   ...
   obs_in = data.shape[0]
   data.drop(data[data.Lev < source['OTHER']['max_lev']].index, axis=0, inplace=True)
   if ENVVARS['DEBUG']: print('   Filter Above Level', obs_in - data.shape[0])
   if data.empty: return pd.DataFrame(), nin, code_error

   # 2) Under pressure level
   obs_in = data.shape[0]
   data.drop(data[data.Lev > source['OTHER']['min_lev']].index, axis=0, inplace=True)
   if ENVVARS['DEBUG']: print('   Filter Above Level', obs_in - data.shape[0])
   if data.empty: return pd.DataFrame(), nin, code_error

   # Write raw data for monitoring
   if monit_file is not None:
      ut.do_monit(data, source, file_data['slot'], 'REPO', monit_file)
...
def get_data(source, files, monit_file):

   OBSPROC = int(ENVVARS['OBSPROC'])

   nin = 0
   code_error = 0
   arg_list = [(row, source, monit_file) for index, row in files.iterrows()]
...
def proc_slot(source, slot_data, pathout, slot, slot_date, monit_file):
   ...
   # Apply filters
   obs_in = slot_data.shape[0]
   slot_data = ut.filter_duplicates(slot_data, ['Lon', 'Lat', 'Lev'])
   ...
   if source['SO/TH']:
      ...
      slot_data = so_th(source, slot_data, [DOMAIN['lat_s'], DOMAIN['lat_n']], [DOMAIN['lon_w'], DOMAIN['lon_e']], [DOMAIN['bottom'], DOMAIN['top']])

   # Write processed data for monitoring
   if monit_file:
      ut.do_monit(slot_data, source, slot, 'PROC', monit_file)

   # Standard data format
   slot_data = ut.standard_data(slot_data, set(column_write).intersection(slot_data.columns))

   # Write data to LETKF binary format
   fileout = f'{pathout}/{MODEL}_{source["NAME"]}_{slot_date:%Y%m%d%H%M%S}.dat'
   ut.write_letkf_dat(source, slot_date, slot_data, fileout, MODEL)
...
def main_asim(args, source):
   ...
   # Get data
   data, nin, exit_code_slot = get_data(source, files, monit_file) 
   ...
   for slot in slots:
      ...
      slot_list.append((source, slot_data, pathout, slot, slot_date, monit_file))

   with mp.Pool(min(OBSPROC, len(slot_list))) as pool:
...
if __name__ == '__main__':
   ...
   print('-----------------------------------')
   time = ut.measure_time(main, sys.argv[1:], src.GEOSND)
