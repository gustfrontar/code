...
def calc_ref_rv(sio, bmap, radar, topo, min_dbz=-20., z=None,  u=None, v=None, w=None, rho=None, qr=None, qs=None, qg=None, tk=None, t=None, dryrun=False , use_wt=True , new_version=True, method=2):
    """
    Calculate radar reflectivity and Doppler velocity
    ...
    qs_ = (sio.readvar('QS', t=t) if qs is None else qs)
    qg_ = (sio.readvar('QG', t=t) if qg is None else qg)
    tk_ = (calc_pt(sio, tout=True, thetaout=False, t=t, dryrun=dryrun)[1] if tk is None else tk)

    if u is None or v is None or w is None:
       u_, v_, w_, _, _, _ = calc_destagger_uvw(sio, t=t, first_grd=False)
    ...
    if z is None:
        z_, _ = calc_height(sio, topo=topo)
    else:
       z_ = z
    ...
    if method == 2:
       ...
       pip = np.pi ** 1.75 #factor
       cf = 1e18 * 720 #factor 

       zr_ = cf * ((rho_ * qr_)**1.75)
       zr_ = zr_ / (pip * (nor_ ** 0.75) * (ror_ ** 1.75))

       # The contribution of snow depends on temperature (bright band effect)
       #mask = np.ma.masked_where(tk_ <= 273.16, tk_).mask
       zs_minor = cf * ki2 * (ros_ ** 0.25) * ((rho_ * qs_) ** 1.75)
       zs_minor = zs_minor / (pip * kr2 * (nos_ ** 0.75) * (roi_ ** 2))
       ...
       zs_major = cf * ((rho_ * qs_) ** 1.75)
       zs_major = zs_major / (pip * (nos_ ** 0.75) * (roi_ ** 1.75))

       zs_ = np.where(tk_ <= 273.16, zs_minor, zs_major)

       # Only dry graupel contribution is ussed.
       zg_ = (cf / (pip * (nog_ ** 0.75) * (rog_ ** 1.75))) ** 0.95
       zg_ = zg_ * ((rho_ * qg_) ** 1.6625)

    # METHOD 3
    elif method == 3: 
       zr_= 2.53e4 * (rho_ * qr_ * 1.0e3) ** 1.84
       zs_= 3.48e3 * (rho_ * qs_ * 1.0e3) ** 1.66
       if new_version :
          zg_= 5.54e3 * (rho_ * qg_ * 1.0e3) ** 1.70
       else :
          zg_= 8.18e4 * (rho_ * qg_ * 1.0e3) ** 1.50

    else:
       print('Method not coded yet'); return

    dbz = 10. * np.log10(zr_ + zs_ + zg_)
    ...
    if method == 2:
       ...
       nos_ = nos_*1e-8      #![cm^-4]
       nog_ = nog_*1e-8      #![cm^-4]
       ror_ = ror_*1e-3      #  ![g/cm3]
       ros_ = ros_*1e-3      #  ![g/cm3]
       rog_ = rog_*1e-3      #![g/cm3] 
    ...
    if use_wt :
       ...
       lr_[mask_] = ((np.pi * ror_ * nor_) / (1.0e-3 * rho_[mask_] * qr_[mask_])) ** 0.25 
       ...
       wg_[mask_] = gamma_ * ((4.0 * gg_ * 100.0 * rog_) / (3.0 * Cd_ * 1.0e-3 * rho_[mask_])) ** 0.5
       wg_[mask_]= 1.0e-2 * wg_[mask_] / (6.0 * lg_[mask_] ** 0.5)

       # Reflectivity weighted terminal velocity
       wt_ = (wr_ * zr_ + ws_ * zs_ +  wg_ * zg_) / (zr_ + zs_ + zg_)

    # Compute azimuth and elevation angle with respect to radar
    if radar['radar_lon'] < 0:
       radar['radar_lon'] += 360.
    ...
    #print('DEBUG CALC RV', rv.max(), rv.min())
    #print('DEBUG CALC REF', dbz.max(), dbz.min())
...
def radar_int(sio, proj, topo, radar, t=None):
   ...
   # Compute model reflectivity and doppler velocity
   ref_, rv_, max_ref_ = calc_ref_rv(sio, bmap, radar, topo,  min_dbz=-20.0, t=t)


   #import matplotlib.pyplot as plt
   #fig, ax = plt.subplots(1,2)
   #ax[0].pcolormesh(ref_[0,:,:], cmap='jet')
   #ax[1].pcolormesh(ref_[10,:,:], cmap='jet')
   #plt.show()

   # Interpolate to radar coordinates
   if sio.bufsize == 0:
       lon_ = np.copy(sio.lon)
       lat_ = np.copy(sio.lat)
   else :
   ...
   rk_ = calc_for.get_k(z_full=np.transpose(z_,axes=[0,2,1]), nlev=nz_, nlon=nx_, nlat=ny_, ri=ri_, rj=rj_, rlev=tz_, nin=np.size(tz_))
   rk_ = rk_ 
   ...
   # Interpolate model data to radar coordinates
   radar['model_rv'] = calc_for.itpl_3d_vec(var=np.transpose(rv_, axes=[0,2,1]), nlev=nz_, nlon=nx_, nlat=ny_, ri=ri_, rj=rj_, rk=rk_, fill_value=undef, nin=nin)

   ref_ = 10.0 ** (ref_ /10.0)
   radar['model_ref'] = calc_for.itpl_3d_vec(var=np.transpose(ref_, axes=[0,2,1]), nlev=nz_, nlon=nx_, nlat=ny_, ri=ri_, rj=rj_, rk=rk_, fill_value=undef, nin=nin)
   radar['model_ref'] = 10.0 * np.log10(radar['model_ref'])

   # Reshape model data
   radar['model_rv'] = radar['model_rv'].reshape(ne_, nr_, na_)
   radar['model_ref'] = radar['model_ref'].reshape(ne_, nr_, na_)

   '''
   import matplotlib.pyplot as plt
   fig, ax = plt.subplots(1,2)
   pc=ax[0].pcolormesh(radar['model_ref'][0,:,:], cmap='jet')
   plt.colorbar(pc, ax=ax[0])
   pc=ax[1].pcolormesh(radar['model_ref'][1,:,:], cmap='jet')
   plt.colorbar(pc, ax=ax[1])
   plt.show()

   fig, ax = plt.subplots(1,2)
   pc=ax[0].pcolormesh(radar['model_rv'][0,:,:], vmin=-10, cmap='jet')
   plt.colorbar(pc, ax=ax[0])
   pc=ax[1].pcolormesh(radar['model_rv'][1,:,:], vmin=-10, cmap='jet')
   plt.colorbar(pc, ax=ax[1])
   plt.show()
   '''
   #print('DEBUG ITPL RV', radar['model_rv'].max(),radar['model_rv'].min())
   #print('DEBUG ITPL REF', np.nanmax(radar['model_ref']), np.nanmin(radar['model_ref']))

   #radar['model_ref2'] = interpn((zc_,yc_,xc_), ref_ , rijk_interp, method='linear', bounds_error=False, fill_value = radar['undef']).reshape(ne_,nr_,na_)
   #radar['model_rv2']  = interpn((zc_,yc_,xc_), rv_  , rijk_interp, method='linear', bounds_error=False, fill_value = radar['undef']).reshape(ne_,nr_,na_)
   #radar['model_ref2'] = 10.0 * np.log10(radar['model_ref2'])

   #print('DEBUG ITPL RV', radar['model_rv2'].max(),radar['model_rv2'].min())
   #print('DEBUG ITPL REF', np.nanmax(radar['model_ref2']), np.nanmin(radar['model_ref2']))
   '''
   import matplotlib.pyplot as plt
   fig, ax = plt.subplots(1,2)
   pc=ax[0].pcolormesh(radar['model_ref2'][0,:,:], cmap='jet')
   plt.colorbar(pc, ax=ax[0])
   pc=ax[1].pcolormesh(radar['model_ref2'][1,:,:], cmap='jet')
   plt.colorbar(pc, ax=ax[1])
   plt.show()

   fig, ax = plt.subplots(1,2)
   pc=ax[0].pcolormesh(radar['model_rv2'][0,:,:], vmin=-10, cmap='jet')
   plt.colorbar(pc, ax=ax[0])
   pc=ax[1].pcolormesh(radar['model_rv2'][1,:,:], vmin=-10, cmap='jet')
   plt.colorbar(pc, ax=ax[1])
   plt.show()
   '''
