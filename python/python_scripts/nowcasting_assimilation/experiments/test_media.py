...
XA = np.zeros([Nx, nens, nvar, 9])
XF = np.zeros([Nx, nens, nvar, 9])
...
for it in range(1,8):
    ...
    UObs[ np.round(ny/2)*nx + np.round(nx/2) +it*it ] = 1*it*it
    VObs[ np.round(ny/2)*nx + np.round(nx/2) +it*it ] = 1*it*it
...
#    plt.pcolormesh(np.reshape(np.mean(XF[:,:,0,it],1),[nx,ny]))
#    plt.colorbar()
#    plt.show()
#    plt.pcolormesh(np.reshape(np.mean(XF[:,:,1,it],1),[nx,ny]))
#    plt.colorbar()
#    plt.show()

    XA[:,:,:,it] = das.da_letkf( nx=Nx , nt=1 , no=NObs , nens=nens ,  xloc=XLoc  ,
                              tloc=np.zeros(Nx)+1.0        , nvar=2                        , xfens=XF[:,:,:,it]           ,
    ...
    xa_mean=np.mean( XA[:,:,:,it-1] , 1 )
    xf_mean=np.mean( XF[:,:,:,it-1] , 1 )

    xa_mean_u=np.reshape( xa_mean[:,0] , [nx,ny] )
    xf_mean_u=np.reshape( xf_mean[:,0] , [nx,ny] )
    print(xa_mean_u.max(), xa_mean_u.min())
    ...
    spread = np.std(XA,axis=1)
    print(np.mean(spread[:,0,it]))
    print(spread.shape)
