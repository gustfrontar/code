...
IF (if_loc(1) == 1 )THEN
   distance_tr(1) = tmp_loc_scale(1) * SQRT(10.0d0/3.0d0) * 2.0d0
ENDIF
IF (if_loc(2) == 1 )THEN
   distance_tr(2) = tmp_loc_scale(2) * SQRT(10.0d0/3.0d0) * 2.0d0
ENDIF
IF (if_loc(1) == 0 )THEN
   distance_tr(1) = 1.0
ENDIF
...
DO io = 1,no
   ...
   IF(  distance(1) < distance_tr(1) .and. distance(2) < distance_tr(2) )THEN
     ...
     Rwf_loc(no_loc)      =  exp(-0.5d0 * (( distance(1)/tmp_loc_scale(1))**2 + &
