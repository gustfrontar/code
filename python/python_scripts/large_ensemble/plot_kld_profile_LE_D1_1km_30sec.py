...
for my_file_type in filetypes     :
  ...
  while ( ctime <= etime ):
    ...
    for my_var in kld  :
        ...
        kld[my_var][kld[my_var]==undef] = np.nan
        kld[my_var][kld[my_var]> 100  ] = np.nan
       ...
       for ilev in range(0,nlev) :
          for var in plot_variables        :
             if var in kld    :
                ...
                vprofile[var][my_mask][ilev,it]=np.nanmean( kld[var][:,:,ilev][ mask[my_mask] ])
...
for my_mask in mask  :
   ...
   for var in vprofile  :
      ...
      cpf.figconf['shadedmin']=0.02
      if var == 'w'     :
         cpf.figconf['shadedmax']=0.1
      elif var == 'qv'  :
         cpf.figconf['shadedmax']=0.1
      elif var == 'dbz' :
         cpf.figconf['shadedmax']=0.4
      else              :
         cpf.figconf['shadedmax']=0.06
      ...
      cpf.figconf['title']='KLD Profile ' + my_mask + ' for var ' + var
      cpf.figconf['figname']='Figure_kld_profile_' + my_mask + '_var_' + var
