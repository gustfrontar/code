...
while ( ctime <= etime ):
   ...
   for my_var in plot_variables   :
       ...
       #for kk in range( 0 , np.shape( kldgr[ my_var ])[2] ) :
   ...
   for my_var in plot_variables :
       ...
       #for kk in range( 0 , np.shape( sprdgr[ my_var ])[2] ) :
   ...
   for ivar,my_var in enumerate(plot_variables)  :
      ...
      #Grid lines
      if irow == 1   :
         plt.xlabel('Spread growth rate')

      if icol == 1   :
         plt.ylabel('KLD growth rate')
