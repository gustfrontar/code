...
itime = dt.datetime(2013,7,13,5,1, 0)  #Initial time.
etime = dt.datetime(2013,7,13,5,23, 0)  #End time.
...
for my_file_type in filetypes     :
  ...
  ctl_file = basedir + expname + '/ctl/anal' + my_file_type + '.ctl'
  ...
  grf_regional_mean=dict()
  grf_regional_max=dict()
  grf_regional_min=dict()
  ...
  my_grf=np.zeros([nx,ny,nlev])

  my_grf_sq=np.zeros([nx,ny,nlev])
  ...
  for var in plot_variables        :
    ...
    if var in ctl_dict['var_list']  :
       ...
       grf_regional_mean[var]=np.zeros([ntimes,nregs])
       grf_regional_max[var] =np.zeros([ntimes,nregs])
       grf_regional_min[var] =np.zeros([ntimes,nregs])

       varpos=[i for i,x in enumerate(ctl_dict['var_list']) if x == var][0]
       varsize=int( ctl_dict['var_size'][varpos] )
       if varsize == 0   :
          varsize = 1
       grf_time_mean[var]=np.zeros([nx,ny,varsize])
       grf_time_std[var]=np.zeros([nx,ny,varsize])
  ...
  while ( ctime <= etime ):
   ...
   my_file=basedir + expname + ptime.strftime("%Y%m%d%H%M%S") + '/anal' + my_file_type + '/' + '/moment0002.grd'

   variance_a=ctlr.read_data_grads(my_file,ctl_dict,masked=False)

   #Gues spread at time T+1
   my_file=basedir + expname + ctime.strftime("%Y%m%d%H%M%S") + '/gues' + my_file_type + '/' + '/moment0002.grd'

   variance_g=ctlr.read_data_grads(my_file,ctl_dict,masked=False)

   #Compute the  using the 3rd and 2nd order moments.
   for my_var in variance_a   :
       variance_a[my_var][ variance_a[my_var] == undef ] = np.nan 
       variance_g[my_var][ variance_g[my_var] == undef ] = np.nan
       grf[my_var] = ( variance_g[my_var] - variance_a[my_var] ) / delta.total_seconds()

   #Read the ensemble mean to get the information from the storm location.
   my_file=basedir + expname + ctime.strftime("%Y%m%d%H%M%S") + '/gues' + my_file_type + '/' + '/moment0001.grd'
   ...
   for var in plot_variables        :
      ...
      if var in ctl_dict['var_list']  :
         ...
         #Plot moments.
         my_grf=grf[var]
         my_grf[ my_grf > 100 ] = np.nan #There are som inf values in the reflectivity field.
         my_grf[ my_grf == undef ] = np.nan

         print('G.R. for Var ',var,' ',(np.nanmin(my_grf)),np.nanmax(my_grf))
         ...
         cpf.figconf['shadedmin']=-0.015
         if var == 'w'     :
            cpf.figconf['shadedmax']=0.08
            cpf.figconf['shadedmin']=-0.08
         else              :
            cpf.figconf['shadedmax']=0.015
         ...
         if my_grf.shape[2] > 1 :
             ...
             for il in plotlevels   : 
                cpf.figconf['title']='Sprd. G.R. for variable ' + var + ' at level ' + str( int(ctl_dict['vlevels'][np.asscalar(il)])) + ' at ' + date
                cpf.figconf['figname']='/Figure_GRF_' + var + '_' + date + '_' + str(int(ctl_dict['vlevels'][np.asscalar(il)])) 

                cpf.plot_x_y_cartopy( lon , lat , my_grf[:,:,il] , max_dbz[:,:,il] , my_grf[:,:,il] )

         else                    :
                cpf.figconf['title']='Sprd. G.R. for variable ' + var + ' at ' + date 
                cpf.figconf['figname']='/Figure_GRF_' + var + '_' + date    
     
                cpf.plot_x_y_cartopy( lon , lat , my_grf[:,:] , max_dbz[:,:,0] , my_grf[:,:] )

      #========================================================================================
      #Generate regional averages of the moment.
      #========================================================================================

      grf_regional_mean[var][it,:],grf_regional_max[var][it,:],grf_regional_min[var][it,:]=cmf.get_regional_average_grid(my_grf,xi,xe,yi,ye,zi,ze,undef)

      #=========================================================================================
      #Acumulate the moment statistics in time. 
      #=========================================================================================

      grf_time_mean[var]=grf_time_mean[var] +  my_grf[:,:,:,0]                     #Accumulate the mean.
      grf_time_std[var]=grf_time_std[var]   + np.power(  my_grf[:,:,:,0]  , 2 )    #Accumulate the standard deviation.
  ...
  #for key in grf_time_mean  :
  for var in plot_variables        :

   if var in ctl_dict['var_list']  :

      print("plotting the grf mean")
      ...
      if my_grf.shape[2] > 1 :
          ...
          for il in plotlevels   :
             ...
             my_grf_mean=np.squeeze(grf_time_mean[var][:,:,il]) / ntimes
             my_grf_sprd=np.sqrt( np.squeeze(grf_time_std[var][:,:,il]) / ntimes  - np.power( my_grf_mean,2) / ntimes )

             cpf.figconf['title']='Mean G.R. for variable ' + var + ' at level ' + str( ctl_dict['vlevels'][np.asscalar(il)])
             cpf.figconf['figname']='/Figure_GRF_mean_sprd_' + var + '_' + date + '_' + str(ctl_dict['vlevels'][il])
             cpf.plot_x_y_cartopy( lon , lat , my_grf_mean , my_grf_sprd , my_grf )

      else                    :
          cpf.figconf['title']='Mean G.R. for variable ' + var 
          cpf.figconf['figname']='/Figure_GRF_mean_sprd_' + var + '_' + date
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      for ireg in range(0,nregs) :
         ...
         cpf.set_default()  #Restore defaults
         cpf.figconf['figpath']=plotbasedir + '/time_independent_plots/'
         cpf.figconf['figname']='Figure_grf_reg_' + iregstr + '_var_' + var 
         cpf.figconf['figsize']=(12,10)
         cpf.figconf['title']='Mean G.R. for var ' + var + ' for region ' + reg_name[ireg] 
         cpf.figconf['titlefontsize']=20
         cpf.figconf['labelfontsize']=20
         cpf.figconf['axessize']=[0.1,0.1,0.8,0.8]
         cpf.figconf['gridline']=True
         ...
         time = np.arange( 0 , np.shape(grf_regional_mean[var])[0] ) * delta.total_seconds()
