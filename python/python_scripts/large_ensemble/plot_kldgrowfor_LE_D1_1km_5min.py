...
plot_pressure_levels=np.array([925,500,300]) #np.array([950,500,300])   #Which levels will be plotted
...
for my_file_type in filetypes     :
  ...
  while ( ctime <= etime ):
   ...
   my_file=basedir + expname + ptime.strftime("%Y%m%d%H%M%S") + '/anal' + my_file_type + '/' + '/kldistance.grd'

   kld_a=ctlr.read_data_grads(my_file,ctl_dict,masked=False)

   #Gues spread at time T+1
   my_file=basedir + expname + ctime.strftime("%Y%m%d%H%M%S") + '/gues' + my_file_type + '/' + '/kldistance.grd'

   kld_f=ctlr.read_data_grads(my_file,ctl_dict,masked=False)

   #Compute remove undef values, compute kld growth 
   #and smooth the result.
   for my_var in kld_a   :
       if my_var in plot_variables    :
          kld_a[my_var][ kld_a[my_var] == undef ] = 0 #np.nan 
          kld_f[my_var][ kld_f[my_var] == undef ] = 0 #np.nan
          kld_a[my_var][ kld_a[my_var] >  100   ] = 0 #np.nan
          kld_f[my_var][ kld_f[my_var] >  100   ] = 0 #np.nan
          kldgr[my_var] = ( kld_f[my_var] - kld_a[my_var] ) / delta.total_seconds()
      ...
      if var in ctl_dict['var_list']  :
         ...
         if my_kldgr.shape[2] > 1 :
             ...
             for il in plotlevels   : 
                ...
                cpf.figconf['figname']='/Figure_KLDGRfor_' + var + '_' + date + '_' + str(int(ctl_dict['vlevels'][np.asscalar(il)])) 

                cpf.plot_x_y_cartopy( lon , lat , my_kldgr[:,:,il] , max_dbz[:,:,il] , my_kldgr[:,:,il] )

         else                    :
                cpf.figconf['title']='KLD G.R. for variable ' + var + ' at ' + date 
                cpf.figconf['figname']='/Figure_KLDGRfor_' + var + '_' + date    
   ...
   if var in ctl_dict['var_list']  :
      ...
      if my_kldgr.shape[2] > 1 :
          ...
          for il in plotlevels   :
             ...
             cpf.figconf['figname']='/Figure_KLDGRfor_mean_sprd_' + var + '_' + date + '_' + str(ctl_dict['vlevels'][il])
             cpf.plot_x_y_cartopy( lon , lat , my_kldgr_mean , my_kldgr_sprd , my_kldgr )

      else                    :
          cpf.figconf['title']='Mean KLD G.R. for variable ' + var 
          cpf.figconf['figname']='/Figure_KLDGRfor_mean_sprd_' + var + '_' + date
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      for ireg in range(0,nregs) :
         ...
         cpf.set_default()  #Restore defaults
         cpf.figconf['figpath']=plotbasedir + '/time_independent_plots/'
         cpf.figconf['figname']='Figure_kldgrfor_reg_' + iregstr + '_var_' + var 
         cpf.figconf['figsize']=(12,10)
