...
for iexp , my_exp in enumerate( expnames )  :
   ...
   while ( ctime <= etime ):
      ...
      my_file=basedir + '/' + my_exp + '/' + ptime.strftime("%Y%m%d%H%M%S") + '/guesgp/kldistance.grd'
      ...
      my_file=basedir + '/' + my_exp + '/' + ptime.strftime("%Y%m%d%H%M%S") + '/guesgp/moment0002.grd'
...
plt.show()
plt.savefig( './Figure_kldgrow_spreadgrow_full_correlation.png' , format='png' , dpi=300)
plt.close()
