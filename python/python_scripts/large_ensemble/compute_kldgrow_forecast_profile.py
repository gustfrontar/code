...
for iexp , my_exp in enumerate( expnames ) :
   ...
   for my_file_type in filetypes     :
      ...
      while ( ctime <= etime )  :
         ...
         ptime = ctime - dt.timedelta( seconds = deltad[iexp] )

         my_file=basedir + my_exp + '/' + ptime.strftime("%Y%m%d%H%M%S") + '/analgp/' + '/' + file_name +'.grd'

         #Read all the variables and levels at once
         tmp_parameter_p = ctlr.read_data_grads(  my_file , ctl_dict , undef2nan = True )

         my_file=basedir + my_exp + '/' + ctime.strftime("%Y%m%d%H%M%S") + '/'+ my_file_type + '/' + '/' + file_name +'.grd'

         #Read all the variables and levels at once
         tmp_parameter_c = ctlr.read_data_grads(  my_file , ctl_dict , undef2nan = True ) 

         tmp_parameter=dict()
         for my_var in tmp_parameter_p  :

            tmp_parameter[my_var] = ( tmp_parameter_c[my_var] - tmp_parameter_p[my_var] )/ deltad[iexp]         
      ...
      for my_var in cvars  :
         ...
         my_file=outputdir + '/kldgrow_forecast_' + my_var + '_rain_profile.npz'
         np.savez(my_file, parameter = parameter_rain[my_var] )

         my_file=outputdir + '/kldgrow_forecast_' + my_var + '_norain_profile.npz'
