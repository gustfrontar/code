...
for iexp , my_exp in enumerate( expnames ) :
   ...
   for my_file_type in filetypes     :
      ...
      while ( ctime <= etime )  :
         ...
         my_file=basedir + my_exp + '/' + ctime.strftime("%Y%m%d%H%M%S") + '/analgp/' + '/' + file_name +'.grd'
      ...
      for my_var in cvars  :
         ...
         my_file=outputdir + '/kldgrow_analysis_' + my_var + '_rain_profile.npz'
         np.savez(my_file, parameter = parameter_rain[my_var] )

         my_file=outputdir + '/kldgrow_analysis_' + my_var + '_norain_profile.npz'
