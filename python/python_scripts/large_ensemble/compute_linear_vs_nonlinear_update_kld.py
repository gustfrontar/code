...
for iexp , my_exp in enumerate( expnames ) :
   ...
   for iv in range( len( variable_combination[0] ) ) :   #Loop over selected variable combinations.
      ...
      my_file=outputdir + '/update_comp_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_kld' + '.grd'
      comm.write_data(outfile=my_file,mydata=meankld,nx=nx,ny=ny,nz=nlev,ie=endian,acc=access)
