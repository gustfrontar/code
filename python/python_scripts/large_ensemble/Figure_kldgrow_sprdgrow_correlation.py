...
for iexp , my_exp in enumerate( expnames )  :
   ...
   delta_data = dt.timedelta( seconds=deltas_data[iexp] ) 
   ...
   while ( ctime <= etime ):
      ...
      my_file=basedir + '/' + my_exp + '/' + ptime.strftime("%Y%m%d%H%M%S") + '/analgp/kldistance.grd'
      ...
      for my_var in plot_variables   :
          ...
          kldgr[my_var] = np.squeeze( np.nanmean( np.delete( kld_c[my_var] - kld_p[my_var] ,4,2) , 2 ) ) / delta_data.seconds
          kldgr[ my_var ] = csf.gaussian_smooth_2d( kldgr[my_var] , 2.0 , 5.0 )



      my_file=basedir + '/' + my_exp + '/' + ptime.strftime("%Y%m%d%H%M%S") + '/analgp/moment0002.grd'
      ...
      for my_var in plot_variables :
         ...
         sprdgr[my_var] = np.squeeze( np.nanmean( np.delete( sprd_c[my_var] - sprd_p[my_var] ,4,2), 2 ) )
         sprdgr[ my_var ] = csf.gaussian_smooth_2d( sprdgr[my_var] , 2.0 , 5.0 ) / delta_data.seconds
      ...
      for my_var in plot_variables :
         ...
         ybar = np.reshape( kldgr[my_var] , ( 1 , np.size( kldgr[my_var] ) ) )
         xbar = np.reshape( sprdgr[my_var] , ( 1 , np.size( kldgr[my_var] ) ) )
...
colors=['#000099','#009900','#cc3300','#ff9900','#66ffff','#cc3399']
...
for ivar,my_var in enumerate(plot_variables)  :
   ...
   for iexp , my_exp in enumerate( expnames ) :
       p=ax.plot( time_index , corrcoef_rain[my_exp][my_var] , color=colors[iexp] ,linestyle='solid' )
       p=ax.plot( time_index , corrcoef_norain[my_exp][my_var] , color=colors[iexp] ,linestyle='dashed' )
...
#plt.show()
plt.savefig( './Figure_kldgrow_spreadgrow_correlation.png' , format='png' , dpi=300)
plt.close()
