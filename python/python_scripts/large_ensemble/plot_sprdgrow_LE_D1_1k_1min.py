...
for my_file_type in filetypes     :
  ...
  sprdgr_regional_mean=dict()
  sprdgr_regional_max=dict()
  sprdgr_regional_min=dict()

  sprdgr_time_mean=dict()
  sprdgr_time_std=dict()

  max_dbz=np.zeros([nx,ny,nlev])

  time_mean_max_dbz=np.zeros([nx,ny,nlev])

  my_sprdgr=np.zeros([nx,ny,nlev])

  my_sprdgr_sq=np.zeros([nx,ny,nlev])
  ...
  for var in plot_variables        :
    ...
    if var in ctl_dict['var_list']  :
       ...
       sprdgr_regional_mean[var]=np.zeros([ntimes,nregs])
       sprdgr_regional_max[var] =np.zeros([ntimes,nregs])
       sprdgr_regional_min[var] =np.zeros([ntimes,nregs])

       varpos=[i for i,x in enumerate(ctl_dict['var_list']) if x == var][0]
       varsize=int( ctl_dict['var_size'][varpos] )
       if varsize == 0   :
          varsize = 1
       sprdgr_time_mean[var]=np.zeros([nx,ny,varsize])
       sprdgr_time_std[var]=np.zeros([nx,ny,varsize])
  ...
  while ( ctime <= etime ):
   ...
   #Compute remove undef values, compute sprd growth 
   #and smooth the result.
   for my_var in sprd_c   :
       if my_var in plot_variables    :
          sprd_c[my_var][ sprd_c[my_var] == undef ] = np.nan 
          sprd_p[my_var][ sprd_p[my_var] == undef ] = np.nan

          my_mask = np.logical_not( np.isnan(sprd_c[my_var] ) )
          sprd_c[my_var][ my_mask ] = np.sqrt( sprd_c[ my_var ][ my_mask ] )
          sprd_c[my_var][ np.logical_not( my_mask ) ] = 0.0

          my_mask = np.logical_not( np.isnan(sprd_p[my_var] ) )
          sprd_p[my_var][ my_mask ] = np.sqrt( sprd_p[my_var][ my_mask ] )
          sprd_p[my_var][ np.logical_not( my_mask ) ] = 0.0

          sprdgr[my_var] = ( sprd_c[my_var] - sprd_p[my_var] ) / delta.total_seconds()

          #Smooth the growing rate.
          sprdgr[my_var][:,:,:,0]=csf.gaussian_smooth_2d( sprdgr[my_var] , 2.0 , 5.0 )
   ...
   for var in plot_variables        :
      ...
      if var in ctl_dict['var_list']  :
         ...
         #Plot moments.
         my_sprdgr=sprdgr[var]
         my_sprdgr[ my_sprdgr > 100 ] = np.nan #There are som inf values in the reflectivity field.
         my_sprdgr[ my_sprdgr == undef ] = np.nan

         print('G.R. for Var ',var,' ',(np.nanmin(my_sprdgr)),np.nanmax(my_sprdgr))
         ...
         if my_sprdgr.shape[2] > 1 :

             for il in plotlevels   : 
                cpf.figconf['title']='Sprd. Sprd G.R. for variable ' + var + ' at level ' + str( int(ctl_dict['vlevels'][np.asscalar(il)])) + ' at ' + date
                cpf.figconf['figname']='/Figure_SPRDGR_' + var + '_' + date + '_' + str(int(ctl_dict['vlevels'][np.asscalar(il)])) 

                cpf.plot_x_y_cartopy( lon , lat , my_sprdgr[:,:,il] , max_dbz[:,:,il] , my_sprdgr[:,:,il] )

         else                    :
                cpf.figconf['title']='SPRD G.R. for variable ' + var + ' at ' + date 
                cpf.figconf['figname']='/Figure_SPRDGR_' + var + '_' + date    
     
                cpf.plot_x_y_cartopy( lon , lat , my_sprdgr[:,:] , max_dbz[:,:,0] , my_sprdgr[:,:] )

      #========================================================================================
      #Generate regional averages of the moment.
      #========================================================================================

      sprdgr_regional_mean[var][it,:],sprdgr_regional_max[var][it,:],sprdgr_regional_min[var][it,:]=cmf.get_regional_average_grid(my_sprdgr,xi,xe,yi,ye,zi,ze,undef)

      #=========================================================================================
      #Acumulate the moment statistics in time. 
      #=========================================================================================

      sprdgr_time_mean[var]=sprdgr_time_mean[var] +  my_sprdgr[:,:,:,0]                     #Accumulate the mean.
      sprdgr_time_std[var]=sprdgr_time_std[var]   + np.power(  my_sprdgr[:,:,:,0]  , 2 )    #Accumulate the standard deviation.
  ...
  #for key in sprdgr_time_mean  :
  for var in plot_variables        :

   if var in ctl_dict['var_list']  :

      print("plotting the sprd gr mean")
      ...
      if my_sprdgr.shape[2] > 1 :
          ...
          for il in plotlevels   :
             ...
             my_sprdgr_mean=np.squeeze(sprdgr_time_mean[var][:,:,il]) / ntimes
             my_sprdgr_sprd=np.sqrt( np.squeeze(sprdgr_time_std[var][:,:,il]) / ntimes  - np.power( my_sprdgr_mean,2) / ntimes )

             cpf.figconf['title']='Mean SPRD G.R. for variable ' + var + ' at level ' + str( ctl_dict['vlevels'][np.asscalar(il)])
             cpf.figconf['figname']='/Figure_SPRDGR_mean_sprd_' + var + '_' + date + '_' + str(ctl_dict['vlevels'][il])
             cpf.plot_x_y_cartopy( lon , lat , my_sprdgr_mean , my_sprdgr_sprd , my_sprdgr )

      else                    :
          cpf.figconf['title']='Mean SPRD G.R. for variable ' + var 
          cpf.figconf['figname']='/Figure_SPRDGR_mean_sprd_' + var + '_' + date
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      for ireg in range(0,nregs) :
         ...
         cpf.set_default()  #Restore defaults
         cpf.figconf['figpath']=plotbasedir + '/time_independent_plots/'
         cpf.figconf['figname']='Figure_sprdgr_reg_' + iregstr + '_var_' + var 
         cpf.figconf['figsize']=(12,10)
         cpf.figconf['title']='Mean SPRD G.R. for var ' + var + ' for region ' + reg_name[ireg] 
         cpf.figconf['titlefontsize']=20
         cpf.figconf['labelfontsize']=20
         cpf.figconf['axessize']=[0.1,0.1,0.8,0.8]
         cpf.figconf['gridline']=True
         ...
         time = np.arange( 0 , np.shape(sprdgr_regional_mean[var])[0] ) * delta.total_seconds()

         cpf.plot_lines( time , sprdgr_regional_mean[var][:,ireg] )
