...
while ( ctime <= etime ):
   ...
   for my_var in plot_variables   :
       ...
       kldgr[my_var] = 1000 * np.squeeze( np.nanmean( kld_c[my_var] - kld_p[my_var] ,2)  ) / delta_data.total_seconds()
   ...
   for my_var in plot_variables :
       ...
       sprdgr[my_var] = np.squeeze( np.nanmean( sprd_c[my_var] - sprd_p[my_var] ,2)  ) / delta_data.total_seconds()
