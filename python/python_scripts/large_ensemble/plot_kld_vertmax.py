...
min_kld_value = 0.05 #Values below this wont be shown in the plots.
...
for my_file_type in filetypes     :
  ...
  while ( ctime <= etime ):
   ...
   for my_var in kld  :
       kld[my_var][ kld[my_var] > 100 ] = np.nan     #Filter unreliable KLD estimates.
       kld[my_var][ kld[my_var] == undef ] = np.nan  #Filter undef KLD values.

       #Use the radar mask to define a masked array.
       tmp_max_kld = np.squeeze( np.nanmax(kld[my_var],2) )
       tmp_mask = np.logical_or( tmp_max_kld < min_kld_value , np.logical_not( radar_mask ) )
       max_kld[my_var] = np.ma.masked_array( tmp_max_kld  , mask = tmp_mask )
   ...
   for var in plot_variables        :
      ...
      if var in ctl_dict['var_list']  :
         ...
         cpf.figconf['shadedmin']=min_kld_value
         if var == 'w'     :
            cpf.figconf['shadedmax']=1.0
         else              :
            cpf.figconf['shadedmax']=0.25
         ...
         cpf.figconf['title']='Max KLD for variable ' + var + ' at ' + date 
         cpf.figconf['figname']='/Figure_KLD_vertmax_' + var + '_' + date    
         cpf.figconf['close']=False
         cpf.plot_x_y_cartopy( lon , lat , my_kld[:,:] , max_dbz[:,:] , my_kld[:,:] )
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      cpf.figconf['figname']='/Figure_KLD_vertmax_mean_sprd_' + var + '_' + date
