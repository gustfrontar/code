...
obs_increment=1.0
variable_combination=[['dbz','dbz','dbz','dbz','v' ,'v' ,'v' ],
                      ['tk' ,'v'  ,'qv' ,'w'  ,'tk','qv','w' ]]
...
for iv in range( len( variable_combination[0] ) ) :   #Loop over selected variable combinations.
   ...
   for iexp , my_exp in enumerate(exps)   :
      ...
      my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/update_comp_kld_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment) + '_rain_profile.npz'

      #Remove the fourth element due to post-processing error.
      profile[my_exp] = np.delete( np.load(my_file)['parameter'] , 4 , axis=0 )

      my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/moment0001_w_rain_profile.npz'

      profile_w[my_exp] = np.delete( np.load(my_file)['parameter'] , 4 , axis=0 )
   ...
   smax = -999.0e99
   for my_exp in exps :
     smax=max( smax , np.max( profile[my_exp] ) )


   for iexp,my_exp in enumerate(exps)  : 

      time_index=np.arange( 0 , profile[my_exp].shape[1] ) * 5.0 + 5.0 #Time index in minutes.
      ...
      #The pcolor
      p=ax.pcolor( time_index , -np.log( levels ) ,  np.squeeze( profile[my_exp] ) 
   ...
   plt.show()
   plt.savefig( 'Figure_kld_profiles_update_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment) + '_rain_profile.png' , format='png' , dpi=300)
   plt.close()
