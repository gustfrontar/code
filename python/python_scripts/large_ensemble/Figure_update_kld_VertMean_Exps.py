...
for iv in range( len( variable_combination[0] ) ) :   #Loop over selected variable combinations.
   ...
   for iexp , my_exp in enumerate(exps)   :
      ...
      my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/update_comp_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_kld' + '.grd'

      kld[my_exp]= np.nanmean( np.delete( ctlr.read_data(my_file,ctl_dict,undef2nan=False) , 4 , 2 ), 2 )
      ...
      kld[my_exp]=cf.gaussian_filter(field0=kld[my_exp],dx=1.0,sigma=sigma_smooth,nx=nx,ny=ny,undef=undef)
      ...
      ensemble_mean[my_exp]= np.delete( ctlr.read_data_grads(my_file,ctl_dict_2,undef2nan=False)['dbz'] , 4 ,2 )
      ...
      print( my_exp )
      print( np.nanmin( ensemble_mean[my_exp]) , np.nanmax( ensemble_mean[my_exp]) )
   ...
   for iexp,my_exp in enumerate(exps)  :
      ...
      print( my_exp)
      print( 'KLD prev per',np.nanmin(kld[my_exp]),np.nanmax(kld[my_exp]) )


      if iexp > 0  :

          kld[my_exp] = 100 * ( ( kld[my_exp] - kld[exps[0]] ) / kld[exps[0]] )
      print( 'KLD after per',np.nanmin(kld[my_exp]),np.nanmax(kld[my_exp]) )

      #Apply radar mask
      print('Var Obs ',var_obs,' Var Upd ',var_upd)
      tmp = np.copy(kld[my_exp])
      ...
      print( 'KLD after mask',np.nanmin(kld[my_exp]),np.nanmax(kld[my_exp]) )
   ...
   #plt.show()
   plt.savefig('Figure_update_kld_VertMean_Exps' + '_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '.png'  , format='png' , dpi=300)
   plt.close()
