...
for my_file_type in filetypes     :
  ...
  while ( ctime <= etime ):
   ...
   for my_var in bimodality  :
       ...
       tmp_mean_bimodality = np.squeeze( np.nanmean(bimodality[my_var],2) )

       tmp_mask = np.logical_not( radar_mask ) 
       mean_bimodality[my_var] = np.ma.masked_array( tmp_mean_bimodality  , mask = tmp_mask )
   ...
   for var in plot_variables        :
      ...
      my_bimodality = mean_bimodality[var]
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      print('plotting the bimodality mean for variable ' + var)
      print('Min=' ,np.min(my_bimodality_mean),' Max=',np.max(my_bimodality_mean) )
      ...
      cpf.figconf['figname']='/Figure_BIMODALITY_time_mean_sprd_' + var 
  ...
  for var in plot_variables        :
   ...
   if var in ctl_dict['var_list']  :
      ...
      for ireg in range(0,nregs) :
         ...
         cpf.figconf['ylabel']=['Bimodality index']
