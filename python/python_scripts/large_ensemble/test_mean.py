...
#Compute the temporal mean of a quantity in this case the kld.
...
for iexp , my_exp in enumerate( expnames ) :
   ...
   for my_file_type in filetypes     :
      ...
      while ( ctime <= etime )  :
         ...
         tmp_parameter[:,:,:,it] = np.squeeze( ctlr.read_data(  my_file , ctl_dict  ) )

         print( np.min(tmp_parameter[:,:,:,it]) , np.max( tmp_parameter[:,:,:,it] ) )
