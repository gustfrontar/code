...
npvalues=10
pvalues=np.arange(0,1+1/npvalues,1/npvalues)
...
for ip in range(0,np.size(pvalues))   :
   ...
   for isample in range( 0 , nsamples )   :
      ...
      my_ensemble[0,0,0,:]= (1-pvalues[ip])*sample + pvalues[ip]*sample_sq

      sample_mean[isample,ip]=np.mean( np.squeeze( my_ensemble[0,0,0,:] ) )
      sample_std[isample,ip] =np.std ( np.squeeze( my_ensemble[0,0,0,:] ) )

      [ hist[isample,:,ip] , x ]= np.histogram( np.squeeze( my_ensemble )  , bins=nbins , range=(-4.0,4.0) )
...
plt.savefig( './Figura_metricas.png' )
...
plt.savefig( './Figura_signal2noise.png' )
plt.close()
...
for ip in range(0,np.size(pvalues))   :
  ...
  plt.savefig( './Figura_histogram_' + str(pvalues[ip]) + '.png' )  
  plt.close()
