...
figname='./Figure_update_rmse_vertmean_diff_variables'
...
for iv,my_var in enumerate( obs_variables ) :
   ...
   my_file=basedir + '/LE_D1_1km_5min/time_mean/guesgp/update_comp_meandiff_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_rmse' + '.grd'
   parameter_5MIN.append( np.nanmean( np.delete( ctlr.read_data(my_file,ctl_dict,undef2nan=False) , 4 , 2 ) , 2 ) )
   my_file=basedir + '/LE_D1_1km_30sec/time_mean/guesgp/update_comp_meandiff_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_rmse' + '.grd'
...
fig.subplots_adjust(wspace=0.0,hspace=0.0,bottom=0.05,left=0.03,right=0.92,top=0.97)
...
for ivar,my_var in enumerate(obs_variables) :
   ...
   plot_rmse_5min   = np.squeeze( parameter_5MIN[ivar] )
   plot_rmse_30sec  = np.squeeze( parameter_30SEC[ivar] )

   plot_rmse_diff = 100 * ( ( plot_rmse_30sec - plot_rmse_5min ) / plot_rmse_5min )

   plot_rmse_5min[ np.logical_not( radar_mask )  ] = np.nan
   plot_rmse_diff[ np.logical_not( radar_mask )  ] = np.nan
   ...
   p=ax.contourf(lon , lat ,  np.transpose( np.squeeze( plot_rmse_diff ) ) ,
   ...
   if ivar == 1  :
      clevs = [0.02,0.04,0.06,0.08]
   if ivar == 2  :
      clevs = [0.04,0.08,0.12,0.16]
   if ivar == 3  :
      clevs = [0.01,0.02,0.03,0.04]
   if ivar == 4  :
      clevs = [0.02,0.04,0.06,0.08,0.1]
   if ivar == 5  :
      clevs = [0.005,0.01,0.015,0.020]

   cpos=ax.contour( lon , lat , 100*np.transpose( plot_rmse_5min ) ,  colors='k',linestyles='solid')
...
m.set_array(np.transpose(plot_rmse_diff))
