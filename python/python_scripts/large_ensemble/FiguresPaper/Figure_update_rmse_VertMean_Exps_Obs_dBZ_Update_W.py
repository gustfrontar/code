...
var_obs = variable_combination[0]
var_upd = variable_combination[1]
...
for iexp , my_exp in enumerate(exps)   :
   ...
   my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/update_comp_meandiff_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment) + '_rmse' + '.grd'
...
for iexp,my_exp in enumerate(exps)  : 
   ...
   if iexp == 0 :
      ...
      #my_map = plt.get_cmap('seismic')  #cpf.cmap_discretize('seismic')
      #my_map = colors.LinearSegmentedColormap.from_list(
      #  'trunc({n},{a:.2f},{b:.2f})'.format(n=my_map.name, a=0.2, b=0.8),
      #  my_map(np.linspace(0.2,0.8,21)))
      #cpf.cmap_discretize(my_map,21)
      smin = -100.0
      smax =  100.0
...
#plt.show()
plt.savefig('Figure_update_rmse_VertMean_Exps_Obs_dBZ_Update_W.eps'  , format='eps' , dpi=300)
plt.close()
