...
for iv in range( len( variable_combination[0] ) ) :   #Loop over selected variable combinations.
   ...
   for iexp , my_exp in enumerate(exps)   :
      ...
      my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/update_comp_meandiff_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_bias' + '.grd'

      #Before taking the average I compute the absolute value to avoid cancelation among biases with opposite signs in the vertical.
      #The idea is to see if the experiments overall have smaller biases with respect to the non-linear update.
      bias[my_exp]= np.nanmean( np.delete( ctlr.read_data(my_file,ctl_dict,undef2nan=True) , 4 , 2 ) , 2 )

      bias[my_exp][ np.isnan( bias[my_exp] ) ] = undef
      bias[my_exp]=cf.gaussian_filter(field0=bias[my_exp],dx=1.0,sigma=sigma_smooth,nx=nx,ny=ny,undef=undef)
      bias[my_exp][ bias[my_exp] == undef ] = np.nan


      print('RMSE values')
      print( my_exp )
      print( np.min( bias[my_exp] ) , np.max( bias[my_exp] ) )
   ...
   for iexp,my_exp in enumerate(exps)  :
      ...
      if iexp > 0  :
         ...
         bias[my_exp] = 100 * ( ( np.abs(bias[my_exp]) - np.abs(bias[exps[0]]) ) / np.abs(bias[exps[0]]) )

      #Apply radar mask
      print('Var Obs ',var_obs,' Var Upd ',var_upd)
      tmp = np.copy(bias[my_exp])
   ...
   for iexp,my_exp in enumerate(exps)  : 
      ...
      if iexp == 0 :
         ...
         smin = max( np.nanmax( varsh ) , -np.nanmin( varsh ) )
         smax = -smin
      else         :
   ...
   #plt.show()
   plt.savefig('Figure_update_bias_VertMean_Exps' + '_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '.png'  , format='png' , dpi=300)
   plt.close()
