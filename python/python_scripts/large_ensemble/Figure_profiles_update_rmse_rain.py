...
for iv in range( len( variable_combination[0] ) ) :   #Loop over selected variable combinations.
   ...
   for iexp , my_exp in enumerate(exps)   :
      ...
      my_file=basedir + '/' + my_exp + '/time_mean/'+ filetype + '/update_comp_rmse_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_rain_profile.npz'
   ...
   plt.show()
   plt.savefig( 'Figure_rmse_profiles_update_vobs_' + var_obs + '_upd_obs_' + var_upd + '_ens_size_' + str(nbv) + '_obs_inc_' + str(obs_increment[iv]) + '_rain_profile.png' , format='png' , dpi=300)
   plt.close()
