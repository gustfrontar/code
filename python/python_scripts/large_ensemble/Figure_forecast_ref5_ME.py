...
while ( ctime <= etime ) :
    ...
    while ( cftime <= fe_time ) :
        ...
        for iexp,my_exp in enumerate(exps)  :
           ...
           ax=axs[irow,icol]
           for_dbzmax= radar_grid['data_ave'][10,:,:,2]
           obs_dbzmax= radar_grid['data_ave'][10,:,:,0]
        ...
        #plt.show()
        plt.savefig( 'forecast_ref5_i' + dt.datetime.strftime( fi_time ,'%Y%m%d%H%M%S' ) + '_' + dt.datetime.strftime( cftime ,'%Y%m%d%H%M%S' ) + '.png' , format='png' , dpi=300  )
        plt.close()
