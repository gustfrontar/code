    ...
    if conf['ftype'] == 'restart':
        ...
        if 'dbz' in conf['varout_3d'] or 'max_dbz' in conf['varout_2d']:
            if not dryrun:
                ...
                sys.stdout.flush()
            X['dbz'], X['max_dbz'] = calc_ref_new(sio, t=it, dryrun=dryrun)
...
#                    if ivar != 'z' and (ivar in conf['varout_3d'] or ivar in ['p', 'tk', 'theta', 'rho', 'rhot']):
            if ivar != 'z' and (ivar in conf['varout_3d']):
                ...
                Xitp[ivar] = interp_z(sio, X[ivar], conf['zlevels'], height=X['z'], t=it, extrap=conf['extrap'])
...
def create_ctlfile(sio, conf, nx, ny, nz, t, tint, tskip_a, nto_a, gradsfile, ctlfile):
    ...
    levs = ''
    if (conf['vcoor'] == 'z' and conf['zlevels'] is None) or conf['vcoor'] == 'o' or conf['ftype'] == 'restart_sprd':
        for ilev in range(nz):
            levs += "{0:12.6f}\n".format(sio.z[ilev])
    elif conf['vcoor'] == 'z':
        for ilev in range(nz):
            levs += "{0:12.6f}\n".format(conf['zlevels'][ilev])
    elif conf['vcoor'] == 'p':
        for ilev in range(nz):
