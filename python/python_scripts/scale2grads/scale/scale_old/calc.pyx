...
__all__ = ['set_bmap', 'calc_rotate_winds',
           ...
           'calc_ref_new', 'calc_ref', 'extrap_z_t0', 'extrap_z_pt', 'extrap_p_zt', 'calc_slp', 'calc_rhosfc_psfc']
...
def interp_z(sio, np.ndarray[f_real, ndim=3] var, zlevels=None, height=None, t=None, extrap=False, dryrun=False):
#def interp_z(sio, var, height=None, t=None, extrap=False):
    ...
    cdef int nko = (sio.dimdef['len']['z'][0] if zlevels is None else len(zlevels))
...
#    varshape[0] = sio.dimdef['len']['z'][0]
    ...
    cdef np.ndarray[double, ndim=1] zlevels_ = (sio.z if zlevels is None else np.asarray(zlevels))
        ...
        for i in range(ni):
            ...
            for ko in range(nko):
                for k in range(k1, nk+1):
                    if k == nk:
                        break
                    elif height_[k,j,i] > zlevels_[ko]: # assume an ascending order of zlevels
                        break
                k1 = k
                if k1 == 0:
                    if extrap:
                    ...
                    varout[ko,j,i] = var[k1-1,j,i] + \
                                     (var[k1,j,i] - var[k1-1,j,i]) * (zlevels_[ko] - height_[k1-1,j,i]) / (height_[k1,j,i] - height_[k1-1,j,i])
#            if extrap:
...
def calc_ref_new(sio, min_dbz=-20., tk=None, rho=None, qr=None, qs=None, qg=None, t=None, dryrun=False):
    """
    Calculate radar reflectivity and radial velocity
    ...
    rog = 913.    #[Kg/m3] 
    roi = 917.    #[Kg/m3]
    roo = 1.      #[Kg/m3] Surface air density.
    ...
    zs_minor = cf * ki2 * (ros ** 0.25) * ((rho_ * qs_) ** 1.75)
    zs_minor = zs_minor / (pip * kr2 * (nos ** 0.75) * (roi ** 2))
    ...
    zs_major = zs_major / (pip * (nos ** 0.75) * (roi ** 1.75))

    zs = np.where(tk_ <= 273.16, zs_minor, zs_major)

    # Only dry graupel contribution is ussed.
    zg = (cf / (pip * (nog ** 0.75) * (rog ** 1.75))) ** 0.95
    zg = zg * ((rho_ * qg_) ** 1.6625)
