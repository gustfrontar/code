...
while i<length(FileList)+1
    try
        disp({i, (i/length(FileList))*100})

        file = [Path FileList(i).name];
        [radar] = rainbow5vol(file);

        [radar]=georeference_radar_data(radar, []);
        radar.data(radar.data==-32) = NaN;

        %disp(size(radar.data))

        if size(radar.data,1)~=361;
            i=i+1;
            continue
        end
...
%Agregado el 30/5/16 para evitar los NaN en Python
elev1(elev1==NaN)=-32;
...
if strcmp(radar2.RadarName, 'INTA_Parana');
    filename = ['/ms-36/mrugna/salidas/PAR_',anio,mes,dia,'.mat'];
end
save(filename)
