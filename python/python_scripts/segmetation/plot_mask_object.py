...
selected_object=29          #ID del objeto seleccionado para plotear.
...
for it in objetos['tiempos'][selected_object] :
   fig_name = './mask_object' + str(selected_object) + '_t' + str(it) + '.png'
   ...
   #Defino una mascara que es 1 donde el objeto fue detectado en el tiempo actual
   #y 0 en el resto del dominio.
   mask=np.zeros( (nx,ny) )
   mask_ext=np.zeros( (nx,ny) )
   xmask=objetos['x'][selected_object]
   ymask=objetos['y'][selected_object]
   tmask=objetos['t'][selected_object]
   x_current=xmask[tmask==it]
   y_current=ymask[tmask==it]
   mask[x_current,y_current]=1.0
   mask = mask[ymin:ymax,xmin:xmax]

   xmask_ext=objetos['x_ext'][selected_object]
   ymask_ext=objetos['y_ext'][selected_object]
   tmask_ext=objetos['t_ext'][selected_object]
   x_current_ext=xmask_ext[tmask_ext==it]
   y_current_ext=ymask_ext[tmask_ext==it]
   mask_ext[x_current_ext,y_current_ext]=1.0
   mask_ext = mask_ext[ymin:ymax,xmin:xmax]
   ...
   ax = axs[1]
   p1=ax.pcolor( x_plot , y_plot , mask )
   ax.contour( x_plot , y_plot , mask_ext )
   if np.max( ref ) > 30.0 :
