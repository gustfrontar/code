...
cross_section_width=10      #Ancho del corte vertical (en puntos de retítula)

labelsize_colorbar = 12    #Tamaño de la letra del colorbar
skipv              = 1     #Espaciamiento de los vectores de viento.
arrow_scale_factor = 10   #Factor de escala para los vectores de viento
...
for it in objetos['tiempos'][selected_object] :
   fig_name = './mp_crosslat_object' + str(selected_object) + '_t' + str(it) + '.png'
   ...
   qc  = to_np( getvar(wrf_file,'QCLOUD')   )[:,ymin:ymax,x] * 1.0e3
   qr  = to_np( getvar(wrf_file,'QRAIN')    )[:,ymin:ymax,x] * 1.0e3
   qi  = to_np( getvar(wrf_file,'QICE')     )[:,ymin:ymax,x] * 1.0e3 
   qs  = to_np( getvar(wrf_file,'QSNOW')    )[:,ymin:ymax,x] * 1.0e3
   qg  = to_np( getvar(wrf_file,'QGRAUP')   )[:,ymin:ymax,x] * 1.0e3
   qh  = to_np( getvar(wrf_file,'QHAIL')    )[:,ymin:ymax,x] * 1.0e3
   t   = to_np( getvar(wrf_file,'tc')       )[:,ymin:ymax,x]
   z   = to_np( getvar(wrf_file,'z')        )[:,ymin:ymax,x]
   ref = to_np( getvar(wrf_file,'dbz')      )[:,ymin:ymax,x]
   w   = to_np( getvar(wrf_file,'wa')       )[:,ymin:ymax,x]
   ...
   ######################################################################## 
   x_plot=np.tile(lat[ymin:ymax,x],(nz,1))
   y_plot=np.copy(z)
   ...
   p2=ax.contour( x_plot , y_plot , w , clevs2 , colors='k',linestyles='solid' )
   clevs2=np.array([-25.0,-20.0,-15.0,-10.0]) * scale_factor
   p2=ax.contour( x_plot , y_plot , w , clevs2 , colors='b',linestyles='dashed' )

   ax.quiver(x_plot[0::skipv,0::skipv],y_plot[0::skipv,0::skipv],v[0::skipv,0::skipv],w[0::skipv,0::skipv],scale=100.0*arrow_scale_factor)
   ...
   p3=ax.contour( x_plot , y_plot , t , [-40.0,-20.0,0.0] , colors='k',linestyles='dashed' , linewidths=2.0 )
