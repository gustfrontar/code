...
def plot_pair( pair , plot_path , show = True , tailind = 1000) :
    ...
    zscore_trade = pair['z_scores_trading']
    zscore_ini   = pair['z_scores_ini']
    zscore_end   = pair['z_scores_end']
    possition = pair['Possition']
    possition_return = pair['Possition_return']
    plt.figure()
    ...
    tmp = np.copy( possition_return ) 
    tmp2 = np.copy( zscore_ini )
    tmp3 = np.copy( possition  )

    plt.figure()
    plt.scatter( possition_return , zscore_ini , c=zscore_end )
    plt.xlabel('Return %')
