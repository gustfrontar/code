...
PATH_LETKFQFX='../../copy/FORECAST_EXP_40mem_sfflux_ntpar_nsmooth_fixparinf_qfx/trajectories/';
PATH_LETKFQFXUST='../../copy/FORECAST_EXP_40mem_sfflux_ntpar_nsmooth_fixparinf_qfxust/trajectories/';
PATH_FIGURE='./FIGURES/';
mkdir(PATH_FIGURE);
...
for it=1:nbt
   ...
   %Trajectory plot.
     ...
     plot(CurrentTracks(it).minlon,CurrentTracks(it).minlat,'-ro','LineWidth',2,'MarkerSize',2);
     ...
     plot(LetkfqfxBestTrack.minlon,LetkfqfxBestTrack.minlat,'-ko','LineWidth',2,'MarkerSize',2);
   ...
   %Min pressure comparison.
     ...
     plot((CurrentTracks(it).daten-sf)*24,CurrentTracks(it).minanomf,'-ro','LineWidth',2);
     ...
     plot((LetkfqfxBestTrack.daten-sf)*24, LetkfqfxBestTrack.minanomf,'-ko','LineWidth',2); 
   ...
   %Min pressure comparison.
     ...
     plot((CurrentTracks(it).daten-sf)*24,CurrentTracks(it).maxwind,'-ro','LineWidth',2);
     ...
     plot((LetkfqfxBestTrack.daten-sf)*24, LetkfqfxBestTrack.maxwind,'-ko','LineWidth',2);
    ...
    subplot(2,2,4)
     ...
     plot((LetkfqfxBestTrack.daten-sf)*24, LetkfqfxBestTrack.dist_track/1e3,'-ko','LineWidth',2);
     ...
     print('-depsc',[PATH_FIGURE '/' datestr(sf,'yyyymmddHH') '_' strtrim(CurrentTracks(it).name) '_BestTrackComp_nosmooth.eps'])
