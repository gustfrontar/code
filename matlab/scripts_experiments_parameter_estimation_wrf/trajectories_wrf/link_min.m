...
function [minstruct MASCARA_SIS]=link_min(FIELD,MASCARA_SIS,minstruct,UMBRAL_ASOCIACION,UMBRAL_CERCANIA,MIN_TR)
...
for imin=1:minstruct(it).nminimos;
  ...
  if( ~minstruct(it).remove(imin) )  %Todo esto lo hago solo si el minimo no esta se??alado como un minimo a ser removido de la lista.
           ...
           if( esminimo ) %El camino llego a otro minimo.
               ...
               if tmp_dist < UMBRAL_CERCANIA
                   if( minstruct(it).minval(imin) <= minstruct(it).minval(imin2))
                       ...
                       minstruct(it).mini(imin2)=0; %Evito que el minimo que va a ser eleminado vuelva a ser asociado a otro minimo.
                       minstruct(it).minj(imin2)=0;
                   else
                       ...
                       minstruct(it).mini(imin)=0;
                       minstruct(it).minj(imin)=0;
                   end
...
end %End del if sobre si el minimo estaba se??alado para ser removido de la lista.
    
end %End del for sobre los minimos.
