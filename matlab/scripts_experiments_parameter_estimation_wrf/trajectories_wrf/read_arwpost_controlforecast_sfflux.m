function [U V W PSFC QVAPOR HGT RAINC RAINNC GEOPT HEIGHT TK RH ...
 RH2 U10M V10M SLP MCAPE MCIN HFXF QFXF USTF]=read_arwpost(file,nx,ny,nz) 
...
%          TSK(TSK>1e15)=NaN;
         ...
         HFXF(:,:)=fread(nfile,[nx ny],'single')';
         HFXF(HFXF>1e15)=NaN;
         QFXF(:,:)=fread(nfile,[nx ny],'single')';
         QFXF(QFXF>1e15)=NaN;
         USTF(:,:)=fread(nfile,[nx ny],'single')';
         USTF(USTF>1e15)=NaN;
