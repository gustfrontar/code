function [TrajStruct]=calc_trajectory_fun_forecast(path_exp,startdate,leadtime,timefrec,member)
%Juan Ruiz 2012
...
filename=[config.path '/' datestr(sdn,'yyyymmddHHMM') '/PLEV_M' memberstr '_F' leadstr '.dat' ]

%READ DATA
[U V QVAPOR GEOPT TK U10M V10M SLP(:,:,count) MCAPE MCIN RAINC RAINNC]=read_arwpost_forecast(filename,config.nx,config.ny,config.nz,true);
