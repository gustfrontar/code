function [group TrajStruct AnalysisTrajStruct]=group_trajectories_fun_2(TrajStruct,AnalysisTrajStruct,startdate,leadtime,timefrec)
...
meanwind=NaN(enssize,ntimes,MaxGroup);
maxwind=NaN(enssize,ntimes,MaxGroup);
...
meanwindanalysis=NaN(ntimes,MaxGroup);
maxwindanalysis=NaN(ntimes,MaxGroup);
...
%          end
       ...
       indexasoc=cell(1);
          ...
          %GRUPO CUANDO LA MISMA TRAYECTORIA NO ES PARTE DEL GRUPO.
           ...
           %Elimino la trayectoria del grupo.
                ...
                meanwind(imember,dindex,cgroup)=NaN;
                maxwind(imember,dindex,cgroup)=NaN; 
       ...
       %DESASOCIARLA DEL ANTERIOR (TAMBIEN SI FUERA EL CASO).
        ...
        if( ~isempty(desasociarlist) && nloop < MaxIterations)   %La trayectoria estaba asociada a otro grupo previamente. 
           ...
           %Elimino la trayectoria del grupo.
                ...
                meanwind(imember,dindex,dgroup)=NaN;
                maxwind(imember,dindex,dgroup)=NaN;
       ...
       if( ~isempty(asociarlist))
          ...
          meanwind(imember,index,newgroup)=TrajStruct(imember).trajectories(ii).meanwind(asocindex);
          maxwind(imember,index,newgroup)=TrajStruct(imember).trajectories(ii).maxwind(asocindex);
    ...
    meanwind(imember,index,ngroup)=TrajStruct(imember).trajectories(ii).meanwind;
    maxwind(imember,index,ngroup)=TrajStruct(imember).trajectories(ii).maxwind;
...
%          end
       ...
       indexasoc=cell(1);
          ...
          %GRUPO CUANDO LA MISMA TRAYECTORIA NO ES PARTE DEL GRUPO.
           ...
           %Elimino la trayectoria del grupo.
                ...
                meanwindanalysis(dindex,cgroup)=NaN;
                maxwindanalysis(dindex,cgroup)=NaN;
       ...
       %DESASOCIARLA DEL ANTERIOR (TAMBIEN SI FUERA EL CASO).
        ...
        if( ~isempty(desasociarlist) && nloop < MaxIterations)   %La trayectoria estaba asociada a otro grupo previamente. 
           ...
           %Elimino la trayectoria del grupo.
                ...
                meanwindanalysis(dindex,dgroup)=NaN;
                maxwindanalysis(dindex,dgroup)=NaN;
       ...
       if( ~isempty(asociarlist))
          ...
          meanwindanalysis(index,newgroup)=AnalysisTrajStruct(ii).meanwind(asocindex);
          maxwindanalysis(index,newgroup)=AnalysisTrajStruct(ii).maxwind(asocindex);
    ...
    meanwindanalysis(index,ngroup)=AnalysisTrajStruct(ii).meanwind;
    maxwindanalysis(index,ngroup)=AnalysisTrajStruct(ii).maxwind;
    ...
    %criterios similares a los que aplique en las trayectorias. 
       ...
       if(~isnan(min_cfd)) %Quiere decir que hay 2 grupos que estan cerca en espacio
        ...
        %por defecto.
          ...
          meanwind(:,:,geliminar)=NaN;
          maxwind(:,:,geliminar)=NaN;
          ...
          meanwindanalysis(:,:,geliminar)=NaN;
          maxwindanalysis(:,:,geliminar)=NaN;
...
meanwind(:,:,contador)=meanwind(:,:,ii);
maxwind(:,:,contador)=maxwind(:,:,ii);
...
meanwindanalysis(:,contador)=meanwindanalysis(:,ii);
maxwindanalysis(:,contador)=maxwindanalysis(:,ii);
...
meanwind(:,:,contador+1:end)=[];
maxwind(:,:,contador+1:end)=[];
...
meanwindanalysis(:,contador+1:end)=[];
maxwindanalysis(:,contador+1:end)=[];
...
group.meanwind=meanwind;
group.maxwind=maxwind;
...
group.meanwindanalysis=meanwindanalysis;
group.maxwindanalysis=maxwindanalysis;
