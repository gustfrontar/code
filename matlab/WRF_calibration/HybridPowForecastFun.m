...
function [ CHybridPow RegressionCoefficients ]=HybridPowForecastFun( CPowFor , CPowObs , CDate , PowForDb , PowObsDb  , DatesDb , Order )

%Esta funcion realiza una combinacion de los pronosticos de potencia
...
%INPUTS
%CPowFor: Un vector conteniendo N pronosticos de potencia producida por el
%parque.
%CPowObs: Un vector conteniendo N observaciones de potencia producida por
%el parque.
...
%PowForDb: Un array conteniendo la base de datos historica de pronosticos
%de potencia. Cada fila corresponde a un pronostico y cada
%columna corresponde a un plazo de pronostico. 
%PowObsDb: Contiene los valores de potencia observados que
%corresponden a cada uno de los pronosticos de potencia contenidos en
%PowForDb.
...
%OUTPUT
%CHybridPow un vector conteniendo los pronosticos de potencia que resultan
...
NLead=length(CPowFor);

%==========================================================================
% DETECTO HASTA DONDE TENGO DATOS
%==========================================================================

tmp=find(isnan(CPowObs));
...
%Reducimos la base de datos y nos quedamos solo con los pronosticos que se inicializaron en dias "cercanos" a la fecha del pronostico actual.
 ...
 DoyPowFor=PowForDb(TimeWindowIndex,:);
 DoyPowObs=PowObsDb(TimeWindowIndex,:);
...
%==========================================================================
 ...
 if( ii >= Lead )
   ...
   DoyLeadPowFor=reshape(DoyPowFor(:,min_lead:max_lead),[TmpNLead*NForDb 1]);
   
   DoyLeadPowObs=repmat(squeeze(DoyPowObs(:,Lead)),[TmpNLead 1]);
   for io=2:Order 
      DoyLeadPowObs(:,io)=repmat(squeeze(DoyPowObs(:,Lead-io+1)),[TmpNLead 1]);
   end
    ...
    X=[ DoyLeadPowFor DoyLeadPowObs];
    
    Y=reshape(DoyPowObs(:,min_lead:max_lead),[TmpNLead*NForDb 1]);
   
    RegressionCoefficients(ii,:) = robustfit(X,Y,'bisquare');
   
    
    CHybridPow(ii) = RegressionCoefficients(ii,1) + CPowFor(ii)*RegressionCoefficients(ii,2);

    for io=1:Order

        CHybridPow(ii) = CHybridPow(ii) + CPowObs(Lead-io+1)*RegressionCoefficients(ii,2+io);
