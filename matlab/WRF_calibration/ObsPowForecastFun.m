...
function [ CPowFor RegressionCoefficients ]=ObsPowForecastFun( CPowObs , CDate , PowObsDb  , DatesDb , Order )
...
%PowObsDb: Contiene los valores de potencia observados en la base de datos
%historica.
...
%OUTPUT
%CPowFor un vector conteniendo los pronosticos de potencia que resultan
...
%==========================================================================
 ...
 if( ii >= Lead )
   ...
   for io=2:Order 
    ...
    CPowFor(ii) = RegressionCoefficients(ii,1) ;

    for io=1:Order

        CPowFor(ii) = CPowFor(ii) + CPowObs(Lead-io+1)*RegressionCoefficients(ii,1+io);
