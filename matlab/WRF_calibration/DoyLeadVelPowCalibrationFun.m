...
function [ MeanErrorPow StdErrorPow SampleSize]=DoyLeadVelPowCalibrationFun( CVelFor , CDate , VelForDb , PowForDb , PowObsDb , DatesDb )

%Esta funcion estima el error medio y su desviacion estandard para la
%potencia producida por un pareque eolico en funcion de la epoca del anio,
%el plazo de pronsotico y el valor del viento pronosticado. 
...
%PowForDb y PowObsDb son los analogos a VelForDb y VelObsDb pero para la
%potencia producida por el parque.
...
%==========================================================================
   ...
   DoyLeadPowObs=reshape(DoyPowObs(:,min_lead:max_lead),[TmpNLead*NForDb 1]);
   
   max_vel=CVelFor(ii)+DVel;
   min_vel=CVelFor(ii)-DVel;
   
   VelIndex= DoyLeadVelFor <= max_vel & DoyLeadVelFor >= min_vel ;
   
   DoyLeadPowFor=DoyLeadPowFor(VelIndex);
   DoyLeadPowObs=DoyLeadPowObs(VelIndex);
  
   
   MeanErrorPow(ii)=nanmean( DoyLeadPowFor - DoyLeadPowObs );
   StdErrorPow(ii)=nanstd( DoyLeadPowFor - DoyLeadPowObs );
