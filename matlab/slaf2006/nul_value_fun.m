...
%**************************************************************************
function [nul_value aux hit hit2 false miss p_lluvia]=nul_value_fun(obs,umbral,a,b,c,d)
...
clear aux;
   ...
   for i_umb=1:length(umbral)               %Loop sobre los umbrales
    ...
    p_lluvia(i_umb)=length(find(obs >= umbral(i_umb)))/length(obs);
    n_total=length(obs);
    %Calculo el numero de pronosticos de lluvia en función de alfa.
    n_prono_lluvia=alfa*length(obs);
    
           
    hit(:,i_umb)=p_lluvia(i_umb).*n_prono_lluvia;
    hit2(:,i_umb)=(1-p_lluvia(i_umb))*(n_total-n_prono_lluvia);
    false(:,i_umb)=(1-p_lluvia(i_umb))*n_prono_lluvia;
    miss(:,i_umb)=(p_lluvia(i_umb)).*(n_total-n_prono_lluvia);                   
