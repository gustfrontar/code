...
% Construye reliability diagrams para dos pronï¿½sticos probabilisticos
% indicando la probabilidad climatolï¿½gica y las frecuencias asociadas a
% cada pronostico.
% Construye los ROC diagrams para los distintos pronï¿½sticos y los miembros
...
% pronï¿½stico puramente estocï¿½stico.
...
% el BIAS del mejor pronï¿½stico del ensemble y del peor y el pronï¿½stico de
% la media.
% Proximamente se agregarï¿½ una estimaciï¿½n del valor econï¿½mico utilizando
% una actividad de referencia y una tabla de payoff sencilla de forma tal
% de evaluar los cambios en el valor asociados a la calibraciï¿½n.
%**************************************************************************
...
%verificacion y la variable est_data que me dice a que estaciï¿½n / dia
%corresponden. 
...
if(hora==24)
 ...
 for i=1:length(est_data_24(:,1))
   ...
   if(length(j)==1)
       ...
       prob6(n,:)=prob_cal5_24(i,:);
       prob7(n,:)=prob_cal6_24(i,:);
       ensemble_aux(n,:)=ensemble(j,:);
       obs(n,1)=p24(j,1);
   end   
 end
...
if(hora==48)
 ...
 for i=1:length(est_data_48(:,1))
   ...
   if(length(j)==1)
       ...
       prob6(n,:)=prob_cal5_48(i,:);
       prob7(n,:)=prob_cal6_48(i,:);
       ensemble_aux(n,:)=ensemble(j,:);
       obs(n,1)=p48(j,1);
   end   
 end
...
[reliability_6 n_forecast_6 prob_ref clim_prob_6]= reliability_fun(obs,prob6,umbral,prob_int);
[reliability_7 n_forecast_7 prob_ref clim_prob_7]= reliability_fun(obs,prob7,umbral,prob_int);
...
[brier_6]=brier_fun(obs,prob6,umbral);
[brier_7]=brier_fun(obs,prob7,umbral);
...
[hit_6 far_6 area_6 ets_6 bias_6 area_ets6] = roc_fun(obs,prob6,umbral,prob_umb);
[hit_7 far_7 area_7 ets_7 bias_7 area_ets7] = roc_fun(obs,prob7,umbral,prob_umb);
...
%hace usando diferentes umbrales de pronï¿½stico para un mismo umbral de
%precipitacion (i.e. manipulando el BIAS del pronï¿½stico deterministico) de
%esta manera obtenemos una curva similar a la del pronï¿½stico
%probabilï¿½stico. Para mayores detalles ver descripciï¿½n en la funciï¿½n
%roc_fun2.
...
%Busco para cada umbral el miembro con la mï¿½xima area del ROC y del ETS
...
save(archivo_salida,'reliability','reliability_2','reliability_3','reliability_4','reliability_5'...
    ,'reliability_6','reliability_7'...
    ,'n_forecast','n_forecast_2','n_forecast_4','n_forecast_5','n_forecast_6','n_forecast_7'...
    ,'n_forecast_3','clim_prob','brier','brier_2','brier_5','brier_6','brier_7'...
    ...
    ,'hit_5','far_5','area_5','ets_5','bias_5'...
    ,'umbral','prob_ref','area_ets','area_ets2','area_ets3','area_ets4','area_ets5','reliability_inf','reliability_sup','brier_inf','brier_sup'...
    ,'brier_clim','brier_ens')
    %'ets_max','ets_min','bias_max','bias_min','hit_max','hit_min','far_max','far_min'...
    %,'ets_mean','bias_mean','hit_mean','far_mean','spread','mean_mse','area_det','area_ets_det')
