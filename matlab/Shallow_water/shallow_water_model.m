...
dt_mins              = 1/FACTOR;   % Timestep (minutes)
...
nx=254*FACTOR; % Number of zonal gridpoints
ny=50*FACTOR;  % Number of meridional gridpoints

dx=100.0e3/FACTOR; % Zonal grid spacing (m)
...
switch initial_conditions
  ...
  case ZONAL_JET
    ...
    convergence_v=conv_v*(-tanh(200.0.*((Y-mean(y))./max(y))).*400);
  case REANALYSIS
    load reanalysis.mat
