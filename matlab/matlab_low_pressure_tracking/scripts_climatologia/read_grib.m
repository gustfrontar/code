...
YREV=false;    %If true then flip the data around the equator.
%INITIALIZE OUTPUT.
...
% 
 ...
 elseif(gribversion==2)
 inventory.lat_min=str2double(tmp{2});
 inventory.lat_max=str2double(tmp{4});
 end
...
elseif(gribversion==2)
 inventory.delta_lat=str2double(tmp{6});
end
