...
if( ~isfield(config,'intervalo'))
    config.intervalo=12;                %Intervalo en horas cada cuanto estan los campos.
end
if( ~isfield(config,'umbral_distancia'))
    config.umbral_distancia=600*1e3*config.intervalo/6; %A partir de que distancia vamos a decir que un sistema es el mismo que 6 horas antes.
end
if( ~isfield(config,'umbral_anom_posicion'))
    config.umbral_anom_posicion=10; %La posicion del sistema se calcula usando el centroide de los puntos cuya anomalia es hasta config.umbral_anom_posicion por encima 
                                %del valor del minimo.
end
...
if( ~isfield(config,'umbral_cercania'))
    config.umbral_cercania=1500e3; %A partir de que distancia vamos a decir que dos
                                   %minimos son un mismo sistema o son sistemas diferentes.
end
if( ~isfield(config,'config.wexp'))      %Exponente para calcular la posicion del sistema mean(lat*anom^config.wexp) cuanto mas grande mas peso tiene la posicion del minimo.
    config.wexp=1;
end

if( ~isfield(config,'config.asymetric_propagation'))      %Si la propagacion de los sistemas se va a considerar igualmente probable en 
                                                          %cualquier direccion o si van a existir direcciones preferenciales
    config.asymetric_propagation=true;
end
if( ~isfield(config,'config.max_eastpropagation'))      %Maxima propagacion permitida hacia el este.
    config.max_eastpropagation=config.umbral_distancia;
end
if( ~isfield(config,'config.max_westpropagation'))      %Maxima propagacion permitida hacia el oeste. (nomralmente menor que la permitida hacia el este).
    config.max_westpropagation=config.umbral_distancia*0.2;
end
if( ~isfield(config,'config.max_nspropagation'))                     %Maxima propagacion permitida hacia el este.
    config.max_nspropagation=config.umbral_distancia*0.9;
end
