...
config.grouppath=['../RESULTS/' config.model '/GROUPVORT/'];
config.minpath=['../RESULTS/' config.model '/MINIMOS/'];
config.forecastlength=7;
config.leadtime=168;      %Requested lead time.      
config.data_pathfor=['/media/GUSTFRONT/data/TIGGE/' config.model '/SMALLGRIB/'];
config.data_pathana=['../DATA/REA2NCEP/HGTVPO/'];
config.result_pathana=['../RESULTS/ANALISIS/'];
config.result_pathfor=['../RESULTS/' config.model '/'];
config.clim_pathana='../RESULTS/CLIMATOLOGY/NCEP500HPA_2.5/';
config.clim_pathfor='../RESULTS/CLIMATOLOGY/NCEP500HPA_2/';
...
load_file=['../RESULTS/' config.model '/GROUPVORT/GROUP_' config.date_ini '_L' num2str(config.forecastlength) '.mat'];
load(load_file);

%DEFINIMOS LAT Y LON

lat_analysis=-90:2.5:90;
lon_analysis=0:2.5:357.5;
...
while ( cur_date_num <= end_date_num )
    ...
    GribFile=[config.data_pathana '/REA2NCEP_HGT_' datestr(cur_date_num,config.datadateformat) config.datasufix];
    ...
    %Call system identification routines.
     ...
     fid=fopen(ClimFile);
     climatology_analysis(:,:,count)=fread(fid,[73 144],'single');
     fclose(fid);
     fid=fopen(DataTmpFile);
     fread(fid,1,'single');
     tmp=fread(fid,[144 73],'single')';
     geopotencial_analysis(:,:,count)=flipdim(tmp,1);
    
    %LEO LOS MINIMOS DEL ANALYSIS.
    MinFileName=[config.result_pathana '/MINIMOS/' datestr(cur_date_num,config.datadateformat) '.bin'];
    ...
    minlatanalysis{count}=tmpminstruct.minlatint;
    minlonanalysis{count}=tmpminstruct.minlonint;
    ...
    fclose(fid);
     ...
     config.ensemblemember=ii;
     GribFile=[config.data_pathfor '/' datestr(ini_date_num,config.datadateformat) '/HGT_500'  datestr(ini_date_num,config.datadateformat) '_F' num2str(int32((count-1)*config.timefrec)) '_M' num2str(int32(config.ensemblemember)) config.datasufix];
     %Decode fields.
     GenLatLon=false;
     ...
     config.ensemblemember=ii;  
     MinFileName=[config.result_pathfor '/MINIMOS/' datestr(cur_date_num,config.datadateformat) '_F' num2str(int32((count-1)*config.timefrec)) '_M' num2str(ii) '.bin'];  
     tmpminstruct=read_min_fun(MinFileName);
     %Convierto la estructura de los minimos a mascara.
    tmpmask=zeros(91,180);
      ...
      minlatforecast{ii,count}=tmpminstruct.minlatint;
      minlonforecast{ii,count}=tmpminstruct.minlonint;
     end
