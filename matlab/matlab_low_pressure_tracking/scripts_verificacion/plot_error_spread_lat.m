...
tmpmean=(group.meandistlaterror);
tmpctrl=(group.ctrldistlaterror);
tmpmemb=group.nummembers;
tmpsprd=group.distlatspread;
...
biasdist_total=nanmean(tmpmean,2);

tmp=repmat(biasdist_total,[1 size(tmpmean,2)]);
meandistdebias_total=sqrt(nanmean((tmpmean-tmp).^2,2));
...
index=group.shemi==1;
tmpmean=group.meandistlaterror(:,index);
tmpctrl=group.ctrldistlaterror(:,index);
tmpmemb=group.nummembers(:,index);
tmpsprd=group.distlatspread(:,index);
mask=( isnan(tmpmean) | isnan(tmpctrl) | tmpmemb < enssize*EnsSizeThre );  %Esta mascara considera solo los puntos donde la media y el control estan presentes.
tmpmean(mask)=NaN;
tmpctrl(mask)=NaN;
tmpsprd(mask)=NaN;
...
biasdist_shemi=nanmean(tmpmean,2);

tmp=repmat(biasdist_shemi,[1 size(tmpmean,2)]);
meandistdebias_shemi=sqrt(nanmean((tmpmean-tmp).^2,2));
...
biasdist_nhemi=nanmean(tmpmean,2);

tmp=repmat(biasdist_nhemi,[1 size(tmpmean,2)]);
meandistdebias_nhemi=sqrt(nanmean((tmpmean-tmp).^2,2));
...
title('Latitude distance error');legend('MEAN ERROR','CTRL ERROR','SPREAD');
...
title('Latitude bias');legend('MEAN','CTRL');
...
title('Latitude bias');legend('SH MEAN','NH MEAN','SH CTRL','NH CTRL');
...
title('latgitude bias');legend('WARM MEAN','COLD MEAN','WARM CTRL','COLD CTRL');
