function fast_read_vorticity_grib(gribfile,matchinput,tmpfile)
...
% 
 ...
 fid=fopen(tmpfile,'r');
 fread(fid,[1],'single');
 geop=fread(fid,[size(latitudes')],'single')';
 fclose(fid);
 
 resolucion=longitudes(1,2)-longitudes(1,1);
 [tmp1,tmp2]=gradient(geop);
 laplaciano=-divergence(tmp1,tmp2)/(resolucion^2);
