function group=group_error_spread_fun(group)
...
group.meananomerror=(squeeze(nanmean(group.minanomsis,1))-group.minanomsisanalysis);
group.meananomspread=(squeeze(nanstd(group.minanomsis)));
tmp=NaN(enssize,flength,ngroup);
for i=1:enssize
   tmp(i,:,:)=group.minanomsisanalysis; 
end
group.meananomrank=sum(tmp > group.minanomsis,1)+1;

group.meanareaerror=(squeeze(nanmean(group.minarea,1))-group.minareaanalysis);
group.meanareaspread=(squeeze(nanstd(group.minarea)));
tmp=NaN(enssize,flength,ngroup);

for i=1:enssize
   tmp(i,:,:)=group.minareaanalysis; 
end
group.meanarearank=sum(tmp > group.minarea,1)+1;
