clear all
close all
%En este test vamos a usar los pronosticos del ECMWF como verificacion. (no
%intenten esto en sus casas!!)
...
startdate='2007110112';     %Fecha inicial del calculo.
enddate  ='2007111512';     %Fecha final del calculo.
config.model='kwbc';
...
load(['../../RESULTS/kwbc/TRAJECTORIES/TRAJ_' config.date_ini '_L' num2str(int32(config.leadtime/24)) '.mat']); 
AnalysisTrajStruct=EnsTrajStruct(1).trajectories;
...
save([config.result_path '/GROUP/GROUPTEST_' config.date_ini '_L' num2str(int32(config.leadtime/24)) '.mat'],'group');
