...
!=============================================
PUBLIC
INTEGER, PARAMETER :: nx=144 !180 !Grid dimensions. 
INTEGER, PARAMETER :: ny=73  !91  !Grid dimensions.
...
REAL   , PARAMETER :: UMB_ANOM=-2.5
REAL   , PARAMETER :: UMB_MINBARRIER=1.25
REAL   , PARAMETER :: UMB_CERCANIA=500e3
REAL   , PARAMETER :: INTERP_TYPE=2                !INTERP TIME FOR SUBGRID MINIMUN 1, MEAN, 2 LEASTE SQUARES QUADRATIC INTERPOLATION
...
!OPEN(UNIT=61,FILE=DATAFILE,FORM='UNFORMATTED',ACCESS='SEQUENTIAL')
OPEN(UNIT=61,FILE=DATAFILE,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=nx*ny*iolen) 
   IF(YREV)THEN
    READ(61,rec=1)((FIELD(ny-j+1,i),i=1,nx),j=1,ny)
    ELSE
    READ(61,rec=1)((FIELD(j,i),i=1,nx),j=1,ny)
    ENDIF
CLOSE(61)
    ...
    ENDIF
     ...
     IF(MINLONINT(ii) .GT. MAXVAL(TMPLON) .OR. MINLONINT(ii) .LT. MINVAL(TMPLON)   &
       & .OR. MINLATINT(ii) .GT. MAXVAL(TMPLAT) .OR. MINLATINT(ii) .LT. MINVAL(TMPLAT))THEN
        !Under some circuntances the method can fail and the interpolated minimun will fall
        !outside the area used for the interpolation.
        !In this case keep the position of the grid point at which the minimun was observed in the 
        !first place.
