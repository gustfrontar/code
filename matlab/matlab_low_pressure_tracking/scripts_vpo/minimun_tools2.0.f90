...
REAL, ALLOCATABLE    ::  MINLAT(:),MINLON(:),MINLATINT(:),MINLONINT(:),MINLAP(:)
...
!OPEN(UNIT=61,FILE=DATAFILE,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=nx*ny*iolen) 
   IF(YREV)THEN
    ...
    READ(61,REC=2)((LAPLACIAN(ny-j+1,i),i=1,nx),j=1,ny)
    ELSE
    READ(61,REC=1)((FIELD(j,i),i=1,nx),j=1,ny)
    READ(61,REC=2)((LAPLACIAN(j,i),i=1,nx),j=1,ny)
    ENDIF
