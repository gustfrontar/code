...
for ipar=1:npar
    tic
    par(ipar)=minpar+(ipar-1)*(maxpar-minpar)/npar;
...
for i=1:numstep
    ...
    %xaens(1,:)=xaens(1,:)+error*randn;
    for k1=1:K
        x=xaens(:,k1)';
        for j=1:bst
            x=logistic(x); %+0.1*randn;    
        end
        xfens(:,k1)=x';
       
    end

    xfens(1,:)=xfens(1,:)+error*randn; %Error no sistematico
    %xfens(1,:)=xfens(1,:)-error;    %Error sistematico
...
rmse(ipar)=sqrt(nanmean((statea(:,1)-state(:,1)).^2));
bias(ipar)=nanmean(statea(:,1)-state(:,1));
toc

end
figure
plot(par,rmse,'LineWidth',2);
grid on
figure
plot(par,bias,'LineWidth',2);
grid on
