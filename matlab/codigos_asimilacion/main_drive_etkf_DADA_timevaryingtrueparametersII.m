...
x0=[8.0d0 0.0d0 30.0d0 10.0d0 8.0/3.0 28.0 0.0 0.0 0.0];      % specify initial conditions


%------------------------------------------------------------
% Data assimilation setup
%------------------------------------------------------------
dx0 = [5. 5. 5. 3.0 1/3 3.0 -0.01 -0.005 -0.01];      % initial perturbation from the true
...
sigmapar=[0.6 0.2 0.6 0.1 0.1 0.1]; %Parameter ensemble standar deviation.
amppar=[0.5 0.25 0.5];
...
x=x0;
for i=1:numtrans
    xout=stepit_parameters_trend(x,h2);
    x=xout;
end
...
for i=1:numstep
    for j=1:bst
        xout=stepit_parameters_trend(x,h2);
        x=xout;
        x(7:9)=x0(7:9)+amppar*cos(2*pi*((i-1)*bst+j)/(250*bst));
    end
    state(i,:)=x;
end
...
xrdn(7,:)=sigmapar(4)*randn(1,K);
xrdn(8,:)=sigmapar(5)*randn(1,K);
xrdn(9,:)=sigmapar(6)*randn(1,K);
...
for i=1:numstep
    ...
    % Data assimilation 
    [xaens,xabar,alfa_a,v_a]=RUN_ETKF_parestinf(yobs(i,:),xfens,p,dim,K,H,R,alfa_a,v_a);  
...
for ii=1:size(statefens,1)
    covarens(:,:,ii)=cov(squeeze(statefens(ii,:,:))');
    correns(:,:,ii)=corrcoef(squeeze(statefens(ii,:,:))');
end
...
plot(mean(statefens(range,4,:),3),'r-','LineWidth',2);
legend('True parameter','Estimated parameter','Individual ensemble members')
title('X variable as a function of time')
subplot(2,1,2)
hold on
plot(mean(statefens(range,4,:),3),'r-','LineWidth',2);
plot(squeeze(statefens(range,4,:)),'b-','LineWidth',0.5);
plot(mean(statefens(range,4,:),3),'r-','LineWidth',2);
plot(state(range,4),'k-','LineWidth',2);
%plot(state(range,1),'k-','LineWidth',2);
%plot(yobs(range,1),'o','MarkerFaceColor','r');
legend('Estimated parameter mean','Individual members')
title('X variable as a function of time')
