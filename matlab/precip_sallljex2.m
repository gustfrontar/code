...
%         n_day=length(daily_data(:,1));
         
%         num_data(ilat,ilon)=0;
%         promedio=0;
         %Busco sobre todos los datos del dia los que caen en la caja y los
         %cuento con la variable num_data
%         for k=1:n_day
%             if( daily_data(:,1) <= lat_n & daily_data(:,1) > lat_s & daily_data(:,2) <= lon_e & daily_data(:,2) > lon_w & daily_data(:,3) ~= undef)
%             num_data(ilat,ilon)=num_data(ilat,ilon)+1;
%             promedio=promedio+daily_data(:,3);
                    
%             end
%         end
         %Promedio los datos de la caja.
%         if(num_data(ilat,ilon) > 0)
%         daily_field(ilat,ilon)=promedio/num_data(ilat,ilon);
%         end
    ...
    date_num=date_num+1;
    clear aux daily_data %Si no borramos estas variables pueden quedar datos de dias anteriores.
...
%formato en distintas resoluciones.
 ...
 save('archivo_mat','precip_salljex');
