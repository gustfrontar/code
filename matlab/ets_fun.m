...
%**************************************************************************
function [ets,hit_rate,far,total,c_umbral]=ets_fun(obs,forecast,umbral)

%**************************************************************************
%           obs= contiene las observaciones (mm)
%           forecast= contiene los pronosticos (tiene tantas columnas como
%           pronosticos queramos verificar).
...
for i_umb=1:n_umb
    ...
    hits_aux=find(obs >= umbral(i_umb));
    c_umbral(i_umb,iens)=length(hits_aux);
    clear i_hits  
    ...
    total=length(i_nonnan); %Calculo el total de pares.
    hits_random(i_umb,iens)=(hits(i_umb,iens) + misses(i_umb,iens))*( hits(i_umb,iens) + false_alarm(i_umb,iens) )./total;
