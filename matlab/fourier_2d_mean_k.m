...
if (str2num(forecast_time)==str2num('f24'))
forecast_ver_time='12';
offset=1;
end

narch=length(arch)-2;
numero_archivos=0; %Contador que lleva el control de cuantos dias efectivamente se pudo hacer el calculo del espectro.
...
%Guardo los espectros en una matriz, para luego calcular el desvio y la media.
espectra500.control(numero_archivos,:)=kspectra_one_d500.control(:);
espectra500.ensemble(numero_archivos,:)=kspectra_one_d500.ensemble(:);
espectra250.control(numero_archivos,:)=kspectra_one_d250.control(:);
espectra250.ensemble(numero_archivos,:)=kspectra_one_d250.ensemble(:);
...
%Calculo los valores medios y la desviacion de cada uno de los espectros.

espectra500mean.control=mean(espectra500.control); %Promedia por columnas.
espectra500mean.ensemble=mean(espectra500.ensemble); %Promedia por columnas.
espectra250mean.control=mean(espectra250.control);
espectra250mean.ensemble=mean(espectra250.ensemble);

%Divido el sigma por la raiz del numero de datos, eso seria el sigma de la media del espectro que es lo que 
%estamos usando como estimador.
espectra500std.control=std(espectra500.control)/(numero_archivos^0.5); %Calcula desviacion standard por columnas.
espectra500std.ensemble=std(espectra500.ensemble)/(numero_archivos^0.5); 
espectra250std.control=std(espectra250.control)/(numero_archivos^0.5);
espectra250std.ensemble=std(espectra250.ensemble)/(numero_archivos^0.5);
...
iplot=1:corte_i-1
subplot(211);
loglog(lambda(iplot),(espectra500mean.control(iplot)),'r',lambda(iplot),(espectra500mean.ensemble(iplot)),'b');
title ('Espectro unidimensional K500');
set(gca,'XTick',[xthick]);
set(gca,'YTick',[xthick]);
set(gca,'XLim',[krange]);
hold on;
loglog(lambda(iplot),(espectra500mean.control(iplot)+espectra500std.control(iplot)),'--r',lambda(iplot),(espectra500mean.control(iplot)-espectra500std.control(iplot)),'--r');
hold on;
loglog(lambda(iplot),(espectra500mean.ensemble(iplot)+espectra500std.ensemble(iplot)),'--b',lambda(iplot),(espectra500mean.ensemble(iplot)-espectra500std.ensemble(iplot)),'--b');


subplot(212);
semilogx(lambda(iplot),100*espectra500mean.ensemble(iplot)./espectra500mean.control(iplot));
set(gca,'XLim',[krange]);
set(gca,'XTick',[xthick]);
...
subplot(211);
loglog(lambda(1:corte_i-1),(espectra250mean.control(iplot)),'r',lambda(iplot),(espectra250mean.ensemble(iplot)),'b');
title ('Espectro unidimensional K250');
set(gca,'XLim',[krange]);
set(gca,'XTick',[xthick]);
set(gca,'YTick',[xthick]);
hold on;
loglog(lambda(iplot),(espectra250mean.control(iplot)+espectra250std.control(iplot)),'--r',lambda(iplot),(espectra250mean.control(iplot)-espectra250std.control(iplot)),'--r');
hold on;
loglog(lambda(iplot),(espectra250mean.ensemble(iplot)+espectra250std.ensemble(iplot)),'--b',lambda(iplot),(espectra250mean.ensemble(iplot)-espectra250std.ensemble(iplot)),'--b');


subplot(212);
semilogx(lambda(iplot),100*espectra250mean.ensemble(iplot)./espectra250mean.control(iplot));
title ('Relacion ens/ctrl');
set(gca,'XLim',[krange]);
set(gca,'XTick',[xthick]);
set(gca,'YTick',[ythick]);
