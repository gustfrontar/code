...
salidatxt=fopen(strcat('superensemble_prob_sur','.txt'),'wt');

%Este archivo de salida en formato texto contiene los datos de probabilidad
%de precipitacion
salidatxt_s=fopen(strcat('prob_smes_s','.txt'),'wt');
...
i_hoy_24=find(pp_ensemble_24(:,1)==hoy_num & pp_ensemble_24(:,3)<lat_max_s & pp_ensemble_24(:,3)>lat_min_s);
i_hoy_48=find(pp_ensemble_48(:,1)==hoy_num & pp_ensemble_48(:,3)<lat_max_s & pp_ensemble_48(:,3)>lat_min_s);
...
i_archivo_24=find(pp_ensemble_24(:,5) >= 0 & pp_ensemble_24(:,1) < hoy_num & pp_ensemble_24(:,3)<lat_max_s & pp_ensemble_24(:,3)>lat_min_s);
i_archivo_48=find(pp_ensemble_48(:,5) >= 0 & pp_ensemble_48(:,1) < hoy_num & pp_ensemble_48(:,3)<lat_max_s & pp_ensemble_48(:,3)>lat_min_s);
...
close 1
    ...
    % La variable ensemble_arch_24 tiene las observaciones mas los
    % pronosticos
...
path_out=strcat('/WRFV2/wrfsi/domains/operativo/archivo/prob_smes/prob_smes_',hoy_dia,'_S','.txt')
