...
%atraso.
%c=load('/WRFV2/wrfsi/domains/operativo/matlab/fecha_cmorph.txt');
c=b-5;
e=b-6;
d=datevec(c);
f=datevec(e);
...
month=tiempo(5:6);
day=tiempo(7:8);
dia=[ str2num(year) str2num(month) str2num(day)];
...
T_36=num2str(t_36);
T_24=num2str(t_24);
%Genero el nombre de los archivos y los abro
...
arch_wrf2406=strcat(archivo_wrf,tiempo_dos,'18f',T_24(13:14),'.3d.dat');
...
pp_prono_72tot=precipitacion(:,:,1)+precipitacion(:,:,2)+precipitacion(:,:,3)+precipitacion(:,:,4); %Pp acumuluda 
%desde las 12UTC del primer dia hasta las 12UTC del segundo dia de pronostico
pp_prono_60tot=precipitacion(:,:,5)+precipitacion(:,:,6)+precipitacion(:,:,7)+precipitacion(:,:,8); 
pp_prono_48tot=precipitacion(:,:,9)+precipitacion(:,:,10)+precipitacion(:,:,11)+precipitacion(:,:,12); 
pp_prono_36tot=precipitacion(:,:,13)+precipitacion(:,:,14)+precipitacion(:,:,15)+precipitacion(:,:,16); 
pp_prono_24tot=precipitacion(:,:,17)+precipitacion(:,:,18)+precipitacion(:,:,19)+precipitacion(:,:,20); 
...
a=size(pp_acum_wrf);
b=a(4)+1;
pp_acum_wrf(:,:,1,b)=pp_prono_72tot;
pp_acum_wrf(:,:,2,b)=pp_prono_60tot;
pp_acum_wrf(:,:,3,b)=pp_prono_48tot;
pp_acum_wrf(:,:,4,b)=pp_prono_36tot;
pp_acum_wrf(:,:,5,b)=pp_prono_24tot;
fecha_archivo(:,b)=dia';
save pp_wrf.mat pp_acum_wrf fecha_archivo
end
...
pp_acum_wrf(:,:,1,1)=pp_prono_72tot;
pp_acum_wrf(:,:,2,1)=pp_prono_60tot;
pp_acum_wrf(:,:,3,1)=pp_prono_48tot;
pp_acum_wrf(:,:,4,1)=pp_prono_36tot;
pp_acum_wrf(:,:,5,1)=pp_prono_24tot;

pp_acum_wrf(:,:,1,2)=pp_prono_72tot;
pp_acum_wrf(:,:,2,2)=pp_prono_60tot;
pp_acum_wrf(:,:,3,2)=pp_prono_48tot;
pp_acum_wrf(:,:,4,2)=pp_prono_36tot;
pp_acum_wrf(:,:,5,2)=pp_prono_24tot;
fecha_archivo(:,1)=dia';
fecha_archivo(:,2)=dia';
save pp_wrf.mat pp_acum_wrf fecha_archivo
end
...
fecha_archivo(:,b)=dia';
save pp_cmorph.mat pp_acum_cmorph fecha_archivo
end
...
cant_dias=1 %Es la cant de dias q quiero tomar para analizar
defa=pp_acum_cmorph(:,:,a(3)-cant_dias:a(3));
vector_cmorph=reshape(defa,nx*ny*size(defa,3),1);    %Me transforma las matrices de esos dias en un vector
