...
modelo='WRFAR'  %Que modelo vamos a utilizar.
...
%Genero el nombre del archivo de salida (en formato matlab).
archivo_mat=strcat('D:/trabajos/TrabajoSLAF/precipitacion/superensemble/matlab/',modelo,'.mat');
...
while (date_num <= end_date_num)
    ...
    fecha_ayer_num=(date_vec(1)-2000)*1e8+date_vec(2)*1e6+date_vec(3)*1e4+15*1e2;
    ...
    i_aux=find(isnan(squeeze(cpc(:,6)))~=1);
