...
%Resolución de los datos que estamos considerando para la calibracion en
%grados.
...
%Umbral de datos (numero mínimo de datos en las cajas salljex que vamos a
...
n_datos=1; %Cuenta el número de pares que vamos a obtener para la calibracion.
...
while (date_num <= end_date_num)
    ...
    n_ini=n_datos; %Me permite saber cual es el n inicial de este día.
  for ilon=1:nx
      for ilat=1:ny
  ...
  %Una vez que termino cada día, computo un coeficiente de correlacion y un
  %numero de datos asociado a este días. Luego puedo sacar un r total, pero
  %la idea es ver también como varia la correlación espacial entre los
  %campos para diferentes dias. Y ver que días la correlación es mejor y
  %tal vez intentar filtrar los dias donde la correlación es muy mala para
...
hold on; %Grafico sobre el mismo gráfico.

scatter(x_lluvia,y_lluvia,2); %Grafico la recta de regresión (calibracion lineal).
