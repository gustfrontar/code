...
for idato=1:ndatos
  ...
  %muchos puntos.
  check=sum(forecast(idato,:),2);
  if(isnan(obs(idato))==0 & isnan(forecast(idato,1))==0 & check > 0) %Solo aportan aquellos donde el dato no es NaN.
  i_eq=find(forecast(idato,:) == obs(idato));
  %Si existen pronosticos iguales a la verificacion entonces los perturbo.
  if(length(i_eq) > 0 )
      obs(idato)=obs(idato)+(rand-0.5)/100;
      for j=1:length(i_eq)
