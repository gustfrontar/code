...
ibox=2;
dt=4;
%Now we will use the pressure at time t and the time derivative of the
%pressure to find the analogs.
data=data(:,:,29,1461:end);
data(:,:,2,2:end)=data(:,:,:,2:end)-data(:,:,:,1:end-1);
data=data(:,:,:,2:end);
...
data_training_y=data_training(:,:,:,1+dt:end);

data_testing_x=data_testing(:,:,:,1:end-dt);
data_testing_y=data_testing(:,:,:,1+dt:end);
